    ...
    with blob_path.open("wb") as handle:
        ...
        for start in range(0, out, row_chunk):
            ...
            if quant_backend == "metal":
                ...
                quantized = quantize(chunk, axis=0)
            else:
...
def _quantize_nvq_chunk(
    ...
    if target_dtype == "NPQ0-L":
        ...
        return quantize_npq0_l_fixed(
            ...
            device=device if quant_backend in {"cuda", "metal"} else "cpu",
        )
    ...
    if quant_backend in {"cuda", "metal"}:
...
def convert(args: argparse.Namespace) -> None:
    ...
    codebook_config = TensorCodebookTrainingConfig(
        ...
        nvq_native_assignment=(
            ...
            and args.nvq_assignment == "native"
        ),
        nvq1_l_native_assignment=(
            quant_backend in {"cuda", "metal"}
            and args.nvq1_l_assignment == "native"
        ),
    ...
    try:
        ...
        runtime_config: dict[str, Any] = {"model_type": architecture}
        if config_path is not None:
            config_bytes = config_path.read_bytes()
            runtime_assets.append(model_config_asset(config_bytes))
            ...
            runtime_config = dict(template_config)
        else:
            print(
                json.dumps(
                    {
