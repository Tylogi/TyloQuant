...
def parse_args():
    parser = argparse.ArgumentParser(description="TPQ OpenAI 兼容服务 CLI 对话")
    parser.add_argument(
        "--base-url",
        default=os.getenv("TPQ_BASE_URL", "http://127.0.0.1:8000/v1"),
        help="API 根地址（默认：http://127.0.0.1:8000/v1）",
    )
    parser.add_argument("--model", default=os.getenv("TPQ_MODEL"), help="模型名，默认自动获取")
    parser.add_argument("--api-key", default=os.getenv("TPQ_API_KEY", ""), help="可选 API Key")
