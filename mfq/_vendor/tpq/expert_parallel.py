...
class GpuResidentExpertParallel:
    ...
    def __init__(
        ...
        requested_layout = (
            ...
            else os.environ.get("TPQ_EP_LAYOUT", "tensor")
        ).strip().lower()
        if requested_layout not in ("expert", "tensor"):
            raise ValueError(
                "TPQ_EP_LAYOUT must be 'expert' or 'tensor'"
            )
        ...
        if requested_codegemm and not self.codegemm_enabled:
            print(
                "[tpq] CodeGEMM 专家路径不兼容，保留原 v256/D4 算子",
                flush=True,
            )
    ...
    def _check_p2p(self) -> bool:
        ...
        if not self.peer_access:
            print(
                "[tpq] Expert Parallel 未检测到双向 CUDA P2P；"
    ...
    def preload_if_fits(self) -> bool:
        ...
        if not self._check_p2p() or not self._check_capacity():
            print(
                f"[tpq] TP={self.tp_size} 全显存专家未激活："
        ...
        for rank, (device, specs) in enumerate(
            ...
            reserve = float(os.environ.get("TPQ_VRAM_RESERVE_GB", "3.0"))
            fraction = max(
                0.10,
                min(
                    0.99,
        ...
        print(
            f"[tpq] TP={self.tp_size} 专家显存 arena 已分配："
        ...
        if self.codegemm_enabled:
            print(
                "[tpq] 全显存专家采用 CodeGEMM Psumbook 索引布局"
                "（同字节原位重排；RAM 专家布局不变）",
                flush=True,
            )
        started = time.time()
        ...
        self.active = True
        print(
            f"[tpq] TP={self.tp_size} 专家权重写入完成："
    ...
    def prepare_codegemm_graphs(self) -> bool:
        ...
        if (
            ...
            or os.environ.get("TPQ_CODEGEMM_GRAPH", "1") == "0"
            or os.environ.get("TPQ_EP_DIRECT_RETURN", "1") == "0"
        ):
            return False
        ...
        for device in self.devices:
            torch.cuda.synchronize(device)
        print(
            f"[tpq] CodeGEMM CUDA Graph 捕获完成："
    ...
    def _compute_decode_device_routed(
        ...
        overlap_shared = (
            ...
            and self.tp_size > 1
            and os.environ.get("TPQ_EP_OVERLAP_SHARED", "1") != "0"
        )
    ...
    def compute_final_overlap(
        ...
        if (
            os.environ.get("TPQ_EP_OVERLAP_SHARED", "1") == "0"
            or os.environ.get("TPQ_GLM_EP_FINAL_FUSED", "1") == "0"
            or x.shape[0] != 1
            or self.tp_size <= 1
            or not self._device_routing_ready(layer)
        ):
