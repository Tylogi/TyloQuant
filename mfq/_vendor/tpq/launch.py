...
_CPU_AUTOMATIC_ENVIRONMENT = {
    "TPQ_CPU_FUSED": "1",
    "TPQ_CPU_PACKED_SINGLE_TEAM": "1",
    "TPQ_CPU_PACKED_DIRECT_ROWS8": "1",
    "TPQ_CPU_PACKED_FUSED_GATE_UP": "1",
    "TPQ_CPU_PACKED_FUSED_DOWN_REDUCE": "1",
    "TPQ_CPU_BLOCK_FP8_BF16": "1",
    "TPQ_CPU_BLOCK_FP8_ROWS8": "0",
    "TPQ_FULL_RESIDENT": "1",
    "TPQ_PREFETCH": "0",
    "OMP_PROC_BIND": "true",
    "OMP_PLACES": "cores",
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--model", required=True, help="TPQ 模型目录")
    parser.add_argument(
        "--profile",
...
def _apply_automatic_runtime_defaults(
    ...
    if _value(args, preset, "device") != "cpu":
        ...
        return
    for key, value in _CPU_AUTOMATIC_ENVIRONMENT.items():
        os.environ.setdefault(key, value)
    if args.cpu_compile is None:
        os.environ.setdefault("TPQ_CPU_COMPILE", "auto")
...
def _apply_environment(
    ...
    if dense_bf16 is not None:
        os.environ["TPQ_DENSE_BF16"] = dense_bf16
    ...
    if cpu_compile is not None:
        os.environ["TPQ_CPU_COMPILE"] = cpu_compile
...
def _summary(args: argparse.Namespace, preset: Any) -> None:
    ...
    print(
        ...
        f"dense_bf16={os.environ.get('TPQ_DENSE_BF16', 'none')}；"
        ...
        f"cpu_compile={os.environ.get('TPQ_CPU_COMPILE', 'off')}"
...
def _maybe_select_auto_extreme(
    ...
    if extreme_load_workspace_gb is not None:
        auto_environment["TPQ_EXTREME_LOAD_WORKSPACE_GB"] = str(
            extreme_load_workspace_gb
        )
    ...
    if decision.activate:
        ...
        return True
    args._extreme_source = f"否({decision.mode})"
    return False
...
def main(argv: list[str] | None = None) -> None:
    ...
    if requested_extreme:
        if args.tp not in (None, 1):
            raise SystemExit("[tpq-launch] --extreme 当前只支持单卡 tp=1")
        if args.device not in (None, "cuda"):
            raise SystemExit("[tpq-launch] --extreme 需要 --device cuda")
        if args.profile not in ("auto", "ram"):
            raise SystemExit(
                "[tpq-launch] --extreme 使用 RAM+VRAM，不能搭配 resident/parallel"
            )
        if any(
            value is not None
            for value in (
            ...
            raise SystemExit(
                "[tpq-launch] --extreme 自动规划容量，不能同时手工指定 "
    ...
    try:
        ...
        if args.extreme and preset.profile != "ram":
            ...
            and args._auto_extreme_decision.mode == "resident"
        ):
            try:
                preset = resolve_preset(
                    args.model,
