// CodeGEMM-style Psumbook kernels for TPQ's full-resident v256/D4 experts.
//
// The packed layout is [K/16, M] int32: each word contains four consecutive
// uint8 D4 code indices. The byte count is identical to TPQ's row-major
// [M,K/4] representation. RAM-resident experts keep the original layout.

constexpr int TPQ_CODEGEMM_CODES = 256;
constexpr int TPQ_CODEGEMM_VECTOR = 4;
constexpr int TPQ_CODEGEMM_K_TILE = 128;
constexpr int TPQ_CODEGEMM_M_TILE = 2048;
constexpr int TPQ_CODEGEMM_THREADS = 256;

__device__ __forceinline__ float tpq_codegemm_dot4(
...
__global__ void tpq_pack_gu_tensor_shard_kernel(
...
__global__ void tpq_pack_dn_tensor_shard_kernel(
    const uint8_t* __restrict__ source,
    uint32_t* __restrict__ target,
    const int rows,
    const int source_blocks,
    ...
    long local_intermediate)
{
    ...
    TORCH_CHECK(
        ...
        source_dn.size(1) * TPQ_CODEGEMM_VECTOR == global_intermediate,
        "source expert dimensions do not match");
    TORCH_CHECK(
        target_gu.size(0) == 2 * local_intermediate &&
        target_dn.size(0) == source_dn.size(0) &&
        target_dn.size(1) * TPQ_CODEGEMM_VECTOR == local_intermediate,
    ...
    TORCH_CHECK(
        status == cudaSuccess && current == target_gu.get_device() &&
    ...
    tpq_pack_gu_tensor_shard_kernel<<<
        (gu_items + 255) / 256,
        256,
        0,
        stream>>>(
    ...
    tpq_pack_dn_tensor_shard_kernel<<<
        (dn_items + 255) / 256,
        256,
        0,
        stream>>>(
            ...
            (int)(shard_start / TPQ_CODEGEMM_VECTOR),
            (int)target_dn.size(1));
    C10_CUDA_KERNEL_LAUNCH_CHECK();
}

__global__ void tpq_unpack_codegemm_kernel(
...
__global__ void tpq_psumbook_routed_kernel(
    ...
    const long input_stride)
{
    ...
    const int input_tile = blockIdx.y * TPQ_CODEGEMM_K_TILE;
    __shared__ float lut[
        TPQ_CODEGEMM_K_TILE / TPQ_CODEGEMM_VECTOR
    ][TPQ_CODEGEMM_CODES];
    ...
    for (
        int vector = 0;
        vector < TPQ_CODEGEMM_K_TILE / TPQ_CODEGEMM_VECTOR;
        ++vector
    ) {
        lut[vector][code] = tpq_codegemm_dot4(
            codebook + code * TPQ_CODEGEMM_VECTOR,
            input_row + input_tile + vector * TPQ_CODEGEMM_VECTOR);
    }
    __syncthreads();

    const int packed_tile = input_tile / 16;
    const int output_begin =
        blockIdx.x * TPQ_CODEGEMM_M_TILE + threadIdx.x * 2;
    const int output_end =
        min(
            (int)((blockIdx.x + 1) * TPQ_CODEGEMM_M_TILE),
            output_dim);
    for (
        int row = output_begin;
        row < output_end;
        row += TPQ_CODEGEMM_THREADS * 2
    ) {
        float value0 = 0.f;
        float value1 = 0.f;
        #pragma unroll
...
__global__ void tpq_codegemm_swiglu_kernel(
...
__global__ void tpq_codegemm_weighted_kernel(
    ...
    torch::Tensor result)
{
    ...
    TORCH_CHECK(
        hidden % TPQ_CODEGEMM_K_TILE == 0 &&
        intermediate % TPQ_CODEGEMM_K_TILE == 0,
    ...
    cudaMemsetAsync(
        ...
        stream);
    tpq_psumbook_routed_kernel<<<
        dim3(
            (2 * intermediate + TPQ_CODEGEMM_M_TILE - 1) /
                TPQ_CODEGEMM_M_TILE,
            hidden / TPQ_CODEGEMM_K_TILE,
            top_k),
        TPQ_CODEGEMM_THREADS,
        0,
        stream>>>(
    ...
    tpq_psumbook_routed_kernel<<<
        dim3(
            (hidden + TPQ_CODEGEMM_M_TILE - 1) /
                TPQ_CODEGEMM_M_TILE,
            intermediate / TPQ_CODEGEMM_K_TILE,
            top_k),
        TPQ_CODEGEMM_THREADS,
        0,
        stream>>>(
