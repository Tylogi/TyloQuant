...
class HiddenStateStore:
    ...
    def __init__(
        ...
        *,
        ...
        self.dtype = dtype
        self.distributed_single_writer = bool(
            dist.is_available() and dist.is_initialized() and dist.get_world_size() > 1
        )
        self.writer = not self.distributed_single_writer or dist.get_rank() == 0
        ...
        if create and self.distributed_single_writer:
            if self.writer:
                self.array = np.memmap(
                    self.path, mode="w+", dtype=numpy_dtype, shape=shape
                )
            dist.barrier()
            if not self.writer:
                self.array = np.memmap(
                    self.path, mode="r+", dtype=numpy_dtype, shape=shape
                )
        else:
            self.array = np.memmap(
                self.path,
    ...
    def write(self, start: int, value: torch.Tensor) -> None:
        ...
        if self.distributed_single_writer:
            dist.barrier()
    ...
    def discard_cached_pages(self) -> None:
        """Release clean mmap pages after a streamed layer has consumed them."""
