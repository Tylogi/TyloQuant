...
def _selection_for_spec(
    ...
    matches = [value for value in evaluations[name].values() if value.spec == spec]
    if len(matches) != 1:
        raise ValueError(f"tensor {name} has {len(matches)} candidates for NINT spec {spec}")
    value = matches[0]
...
def _strategy_key(strategy: LayerStrategy) -> tuple[tuple[str, int, int, int], ...]:
    return tuple(
        sorted(
...
def _strategy_for_scheme(
    ...
    expected = {name: scheme.require(name).spec for name in strategies[0].specs}
