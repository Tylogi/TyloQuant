...
_MASKED_FORMAT = "mfq.calibration-corpus.v2"
...
class CalibrationBatch:
    ...
    def attention_mask(self) -> np.ndarray:
        ...
        if self.padding_mask.shape != self.input_ids.shape:
            raise ValueError("calibration padding mask differs from input_ids")
        return np.asarray(self.padding_mask, dtype=np.int64)

    @property
    def next_token_loss_mask(self) -> np.ndarray:
        """Return the positions whose next-token targets contribute to the objective."""

        if self.loss_mask is None:
            return np.ones(
                (self.input_ids.shape[0], self.input_ids.shape[1] - 1),
                dtype=np.bool_,
            )
        if self.loss_mask.shape != self.input_ids.shape:
            raise ValueError("calibration loss mask shape differs from input_ids")
        return np.asarray(self.loss_mask[:, 1:], dtype=np.bool_)

    @property
    def scored_positions(self) -> int:
        return int(self.next_token_loss_mask.sum())
...
class CalibrationCorpus:
    ...
    def close(self) -> None:
        values = (self.tokens, self.offsets, self.split_ids, self.domain_ids, self.loss_mask)
        for value in values:
            if value is None:
                continue
    ...
    def chunk_loss_mask(self, index: int) -> np.ndarray:
        if index < 0 or index >= self.chunks:
            raise IndexError(f"calibration chunk {index} is outside [0, {self.chunks})")
        start = int(self.offsets[index])
        end = int(self.offsets[index + 1])
        if self.loss_mask is None:
            return np.ones(end - start, dtype=np.bool_)
        return np.asarray(self.loss_mask[start:end], dtype=np.bool_)

    def scored_token_count(self, split: str | None = None) -> int:
        if self.loss_mask is None:
            if split is None:
                return int(self.tokens.size)
            return self.token_count(split)
        if split is None:
            return int(np.asarray(self.loss_mask, dtype=np.uint8).sum())
        split_id = _split_id(split)
        total = 0
        for index in np.flatnonzero(self.split_ids == split_id):
            start = int(self.offsets[index])
            end = int(self.offsets[index + 1])
            total += int(np.asarray(self.loss_mask[start:end], dtype=np.uint8).sum())
        return total
    ...
    def iter_batches(
        ...
        *,
        ...
        """Yield batches without crossing document chunks.

        By default only records of exactly the same length share a batch.  When
        ``pad_to_multiple`` is set, records are sorted by length after the
        seeded sample selection, grouped into batches, and right-padded.  The
        returned attention and loss masks exclude every padded position.
        """
        ...
        for chunk_index in np.flatnonzero(self.split_ids == split_id):
            ...
            if self.loss_mask is not None and end - start > window_length:
                raise ValueError(
                    "assistant-masked calibration traces cannot be split across windows; "
                    "increase window_length to at least the longest saved trace"
                )
            cursor = start
            while cursor < end:
        ...
        if max_tokens is not None:
            ...
            for item in windows:
                ...
                if remaining < 2:
                    break
                if length > remaining:
                    if drop_last or self.loss_mask is not None:
                        continue
        ...
        if pad_to_multiple is None:
            ...
            # Keep the seeded subset fixed, then minimize padding by grouping
            # adjacent lengths.  A single synthetic bucket preserves the
            # requested batch size instead of degenerating to batch=1 for
            # model-generated conversations whose lengths are nearly unique.
            buckets[0] = sorted(windows, key=lambda item: item[2] - item[1], reverse=True)
        ...
        for length in sorted(buckets, reverse=True):
            ...
            for start in range(0, len(bucket), batch_size):
                ...
                if len(items) < batch_size and drop_last:
                    continue
                lengths = [right - left for _, left, right in items]
                padded = max(lengths)
                if pad_to_multiple is not None:
                    padded = ((padded + pad_to_multiple - 1) // pad_to_multiple) * pad_to_multiple
                batch = np.zeros((len(items), padded), dtype=np.int64)
                padding_mask = np.zeros((len(items), padded), dtype=np.bool_)
                loss_mask = (
                    None
                    if self.loss_mask is None
                    else np.zeros((len(items), padded), dtype=np.bool_)
                )
                for row, (_, left, right) in enumerate(items):
                    size = right - left
                    batch[row, :size] = np.asarray(self.tokens[left:right], dtype=np.int64)
                    padding_mask[row, :size] = True
                    if loss_mask is not None:
                        loss_mask[row, :size] = np.asarray(
                            self.loss_mask[left:right], dtype=np.bool_
                        )
                yield CalibrationBatch(
                    ...
                    loss_mask=loss_mask,
                    padding_mask=(padding_mask if pad_to_multiple is not None else None),
                )
...
def _trace_token_ids(
    ...
    *,
    expected_generator_model: str,
    expected_mode: str | None,
) -> tuple[list[int], list[int], str, str, int | None]:
    ...
    raw_template_kwargs = value.get("chat_template_kwargs", {})
    if not isinstance(raw_template_kwargs, Mapping):
        raise TypeError("trace chat_template_kwargs must be an object")
    template_kwargs: dict[str, Any] = dict(raw_template_kwargs)
    if value.get("tools") is not None:
        template_kwargs["tools"] = value["tools"]
    ...
    if isinstance(prompt_rendered, Mapping):
        prompt_rendered = prompt_rendered["input_ids"]
    if hasattr(prompt_rendered, "tolist"):
        prompt_rendered = prompt_rendered.tolist()
    if prompt_rendered and isinstance(prompt_rendered[0], list):
        prompt_rendered = prompt_rendered[0]
    prompt_ids = [int(token) for token in prompt_rendered]

    exact_prompt = value.get("input_token_ids")
    exact_output = value.get("output_token_ids")
    if (exact_prompt is None) != (exact_output is None):
        raise ValueError("trace must provide both input_token_ids and output_token_ids")
    if exact_prompt is not None:
        if not isinstance(exact_prompt, Sequence) or isinstance(exact_prompt, (str, bytes)):
            raise TypeError("trace input_token_ids must be a sequence")
        if not isinstance(exact_output, Sequence) or isinstance(exact_output, (str, bytes)):
            raise TypeError("trace output_token_ids must be a sequence")
        saved_prompt = [int(token) for token in exact_prompt]
        output_ids = [int(token) for token in exact_output]
        if saved_prompt != prompt_ids:
            raise ValueError("saved trace input_token_ids differ from the pinned chat template")
        if not output_ids:
            raise ValueError("trace output_token_ids must be non-empty")
        ids = saved_prompt + output_ids
        loss_mask = [0] * len(saved_prompt) + [1] * len(output_ids)
    else:
        ...
        boundary = 0
        for prompt_token, full_token in zip(prompt_ids, ids, strict=False):
            if prompt_token != full_token:
                break
            boundary += 1
        if boundary == 0 or boundary >= len(ids):
            raise ValueError("cannot identify the final assistant span in rendered trace")
        loss_mask = [0] * boundary + [1] * (len(ids) - boundary)
    if len(ids) < 2:
        raise ValueError("rendered trace contains fewer than two tokens")
    return ids, loss_mask, mode, source_dataset, declared_tokens
...
def _write_corpus_artifact(
    ...
    manifest: dict[str, Any],
    loss_masks: Sequence[np.ndarray] | None = None,
) -> CalibrationCorpus:
    if not chunks:
    ...
    packed_loss_mask: np.ndarray | None = None
    if loss_masks is not None:
        if len(loss_masks) != len(chunks):
            raise ValueError("calibration loss-mask chunk count mismatch")
        for chunk, mask in zip(chunks, loss_masks, strict=True):
            if mask.ndim != 1 or mask.shape != chunk.shape:
                raise ValueError("calibration loss mask must match its token chunk")
        packed_loss_mask = np.concatenate(loss_masks).astype(np.uint8, copy=False)
        if np.any(packed_loss_mask > 1):
            raise ValueError("calibration loss mask must contain only zero or one")
    ...
    try:
        ...
        if packed_loss_mask is not None:
            np.save(temporary / "loss_mask.npy", packed_loss_mask, allow_pickle=False)
...
def build_trace_corpus_from_jsonl(
    ...
    *,
    ...
    chunks: list[np.ndarray] = []
    chunk_loss_masks: list[np.ndarray] = []
    ...
    scored_tokens = {split: 0 for split in _SPLITS}
    ...
    try:
        for split in ("train", "validation"):
            ...
            for candidate in pools[split]:
                ids, loss_mask, mode, source_dataset, declared_tokens = _read_trace_candidate(
                ...
                if take < 2:
                    continue
                if not any(loss_mask[:take]):
                    continue
                if take != len(ids):
                    partial_records[split] += 1
                chunk = np.asarray(ids[:take], dtype=np.int32)
                chunk_loss_mask = np.asarray(loss_mask[:take], dtype=np.uint8)
                chunks.append(chunk)
                chunk_loss_masks.append(chunk_loss_mask)
                ...
                scored_tokens[split] += int(chunk_loss_mask.sum())
                remaining -= take
                if remaining == 0:
                    break
            if remaining:
    ...
    manifest = {
        ...
        "trace": {
            ...
            "objective_mask": "final_teacher_assistant_output_tokens_only",
            "exact_generation_token_ids_preferred": True,
...
def load_corpus(path: str | Path) -> CalibrationCorpus:
    ...
    corpus_format = manifest.get("format")
    if corpus_format not in {_FORMAT, _MASKED_FORMAT}:
    ...
    loss_mask = None
    if corpus_format == _MASKED_FORMAT:
        loss_mask = np.load(root / "loss_mask.npy", mmap_mode="r", allow_pickle=False)
    if tokens.dtype != np.int32 or offsets.dtype != np.int64:
        raise ValueError("calibration token/offset arrays have invalid dtypes")
    if loss_mask is not None:
        if loss_mask.dtype != np.uint8 or loss_mask.ndim != 1:
            raise ValueError("calibration loss mask has an invalid dtype or shape")
        if loss_mask.size != tokens.size or np.any(loss_mask > 1):
            raise ValueError("calibration loss mask does not cover the token array")
    ...
    corpus = CalibrationCorpus(root, manifest, tokens, offsets, split_ids, domain_ids, loss_mask)
    ...
    for split, expected in manifest.get("actual_scored_tokens", {}).items():
        if corpus.scored_token_count(split) != int(expected):
            raise ValueError(f"calibration split {split} scored-token count does not match manifest")
    return corpus
