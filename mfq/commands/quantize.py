...
def add_parser(subparsers: argparse._SubParsersAction) -> None:
    ...
    precision.add_argument(
        "--imatrix",
        default="",
        help="MFQ-supported GGUF or legacy llama.cpp importance matrix",
    )
    precision.add_argument(
        "--tokenizer",
        default="",
...
def _detect_source(path: Path, requested: str) -> str:
    if requested != "auto":
        return requested
    if path.is_dir():
        if (path / "model.safetensors.index.json").is_file() or any(
...
def _validate(args: argparse.Namespace, source_format: str) -> None:
    ...
    if source_format == "gguf" and (
        args.bits != 4 or args.groupsize != 24 or args.sub_bits != 6
    ):
