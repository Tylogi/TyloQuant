...
class TpqPqSpec:
    ...
    def __post_init__(self) -> None:
        if self.tier not in {"x", "w", "v", "vv", "p"}:
            raise ValueError(f"unsupported TPQ tier: {self.tier!r}")
    ...
    def index_bits(self) -> int:
        # TPQ stores a physical byte/word dtype, while TPQ2 Kimi archives may
        # preserve row-aligned p12/p14 streams.
...
TPQ_X = TpqPqSpec("x", 8, 256)
TPQ_W = TpqPqSpec("w", 8, 4096)
TPQ_V = TpqPqSpec("v", 4, 256)
TPQ_VV = TpqPqSpec("vv", 4, 4096)
...
class TpqPqTensor:
    ...
    def __post_init__(self) -> None:
        ...
        if len(shape) != 2 or any(value <= 0 for value in shape):
            raise ValueError("TPQ-PQ currently requires a positive 2-D matrix")
        if self.axis != 0 or self.neuron_len != shape[1]:
            raise ValueError("TPQ-PQ requires axis=0 and neuron_len=shape[1]")
        if self.neuron_len % self.spec.vector_size:
            raise ValueError(
                f"TPQ-PQ width {self.neuron_len} is not divisible by "
    ...
    def payload_nbytes(self) -> int:
        return tpq_pq_payload_nbytes(self.shape, self.spec)


@dataclass(frozen=True)
class TpqInt4Tensor:
    """TPQ symmetric int4-g64 dense matrix."""
    ...
    def __post_init__(self) -> None:
        shape = tuple(int(value) for value in self.shape)
        if len(shape) != 2 or any(value <= 0 for value in shape):
            raise ValueError("TPQ-I4 currently requires a positive 2-D matrix")
        rows, columns = shape
        if self.axis != 0 or self.neuron_len != columns:
            raise ValueError("TPQ-I4 requires axis=0 and neuron_len=shape[1]")
        if self.group_size <= 0 or columns % self.group_size or columns % 2:
            raise ValueError("TPQ-I4 width must be divisible by group size and two")
        ...
        if tuple(packed.shape) != (rows, columns // 2):
            raise ValueError("TPQ-I4 packed values have the wrong shape")
        if tuple(scales.shape) != (rows, columns // self.group_size):
            raise ValueError("TPQ-I4 scales have the wrong shape")
        if not np.isfinite(scales).all() or np.any(scales < 0):
            raise ValueError("TPQ-I4 scales must be finite and non-negative")
        object.__setattr__(self, "shape", shape)
    ...
    @property
    def payload_nbytes(self) -> int:
        return tpq_int4_payload_nbytes(self.shape, self.group_size)
...
def pack_tpq_indices(values: np.ndarray, bits: int) -> bytes:
...
def unpack_tpq_indices(
    blob: bytes | memoryview,
    offset: int,
    count: int,
    bits: int,
) -> tuple[np.ndarray, int]:
    """Decode one little-endian TPQ index stream."""

    nbytes = (count * bits + 7) // 8
    end = offset + nbytes
    if end > len(blob):
        raise ValueError("truncated TPQ-PQ index stream")
    if bits == 8:
        return (
...
def tpq_pq_payload_nbytes(
    shape: tuple[int, ...],
    spec: TpqPqSpec,
) -> int:
    rows, columns = (int(value) for value in shape)
    if columns % spec.vector_size:
        raise ValueError("TPQ-PQ width must be divisible by its vector size")
...
def pack_tpq_pq_prefix(
    spec: TpqPqSpec,
    shape: tuple[int, int],
    codebook: np.ndarray,
) -> bytes:
    """Pack the fixed metadata preceding a streamed TPQ index payload."""

    rows, columns = (int(value) for value in shape)
    table = np.ascontiguousarray(codebook, dtype=np.float32)
    if tuple(table.shape) != (spec.codebook_entries, spec.vector_size):
        raise ValueError("TPQ-PQ streamed codebook has the wrong shape")
    if columns % spec.vector_size:
        raise ValueError("TPQ-PQ streamed width is not vector aligned")
    return b"".join(
        (
            _PQ_HEADER.pack(
                _PQ_MAGIC,
...
def pack_tpq_pq(tensor: TpqPqTensor) -> bytes:
    """Serialize a TPQ product-VQ matrix."""
...
def unpack_tpq_pq(blob: bytes | memoryview) -> TpqPqTensor:
    """Deserialize a TPQ product-VQ matrix."""

    if len(blob) < _PQ_HEADER.size:
        raise ValueError("truncated TPQ-PQ header")
    (
        magic,
        version,
        tier_id,
    ...
    if magic != _PQ_MAGIC or version != _PQ_VERSION:
        raise ValueError("invalid TPQ-PQ header")
    try:
        tier = _TIER_FROM_ID[int(tier_id)]
    except KeyError as exc:
        raise ValueError(f"unsupported TPQ-PQ tier id: {tier_id}") from exc
    spec = TpqPqSpec(
        tier=tier,
    ...
    if ndim != 2:
        raise ValueError("TPQ-PQ payload must contain a 2-D matrix")
    ...
    offset += 4
    if rows != shape[0]:
        raise ValueError("TPQ-PQ row count does not match its shape")
...
def pack_tpq_int4(tensor: TpqInt4Tensor) -> bytes:
    """Serialize a TPQ symmetric int4 matrix."""

    return b"".join(
        (
            pack_tpq_int4_prefix(tensor.shape, tensor.group_size),
...
def tpq_int4_payload_nbytes(
    shape: tuple[int, ...],
    group_size: int = 64,
) -> int:
    """Return the exact native payload size for one TPQ int4 matrix."""

    if len(shape) != 2:
        raise ValueError("TPQ-I4 payload size requires a 2-D shape")
    rows, columns = (int(value) for value in shape)
    if rows <= 0 or columns <= 0 or columns % group_size or columns % 2:
        raise ValueError("TPQ-I4 payload shape is not group aligned")
    return (
        _INT4_HEADER.size
        + 16
        + _MATRIX_TAIL.size
...
def pack_tpq_int4_prefix(
    shape: tuple[int, ...],
    group_size: int = 64,
) -> bytes:
    """Pack fixed metadata preceding streamed TPQ int4 values."""

    if len(shape) != 2:
        raise ValueError("TPQ-I4 prefix requires a 2-D shape")
    rows, columns = (int(value) for value in shape)
    tpq_int4_payload_nbytes((rows, columns), group_size)
    return b"".join(
        (
            _INT4_HEADER.pack(
                _INT4_MAGIC,
...
def unpack_tpq_int4(blob: bytes | memoryview) -> TpqInt4Tensor:
    """Deserialize a TPQ symmetric int4 matrix."""

    if len(blob) < _INT4_HEADER.size:
        raise ValueError("truncated TPQ-I4 header")
    (
        magic,
        version,
        group_size,
    ...
    if magic != _INT4_MAGIC or version != _INT4_VERSION or ndim != 2:
        raise ValueError("invalid TPQ-I4 header")
    ...
    if rows != shape[0] or neuron_len != shape[1]:
        raise ValueError("TPQ-I4 dimensions are inconsistent")
