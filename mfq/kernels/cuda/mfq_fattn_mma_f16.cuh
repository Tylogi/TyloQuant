...
#pragma unroll
                for (int k0 = k0_start; k0 < k0_stop; k0 += stride_k) {
                    ...
                    cp_async_cg_16<preload>(tile_KV_32 + i*(stride_tile*sizeof(half2)) + k*16, KV + int64_t(row)*stride_KV + k*h2_per_chunk);
                }
            }
        };
...
#pragma unroll
                for (int k0 = k0_start; k0 < k0_stop; k0 += stride_k) {
                    ...
                    ggml_cuda_memcpy_1<16>(tile_KV + i*stride_tile + k*4,
                        valid ? KV + int64_t(row)*stride_KV + k*h2_per_chunk : zero);
                }
            }
        };
        // 1: max 32*16=512 bytes, 256 half
...
template<int DKQ, int DV, int ncols1, int ncols2, int nwarps,
    bool use_logit_softcap, bool V_is_K_view, bool use_indirect,
...
static __device__ __forceinline__ void flash_attn_ext_f16_iter(
        ...
        const int    * const __restrict__ row_indices,
        const half   * const __restrict__ mask_h,
        const bool implicit_causal,
    ...
    if constexpr (nstages > 1) {
        ...
        cp_async_wait_all();
        __syncthreads();
        const half2 * const V_tile = use_indirect
            ? V_h2
            : V_h2 + int64_t(k_VKQ_0)*stride_V;
        flash_attn_ext_f16_load_tile<stride_tile_V, nwarps, nbatch_fa,
...
#pragma unroll
    for (int k0_start = (DKQ/2-1) - (DKQ/2-1) % nbatch_K2; k0_start >= 0; k0_start -= nbatch_K2) {
        ...
        if constexpr (nstages <= 1) {
            ...
            flash_attn_ext_f16_load_tile<stride_tile_K, nwarps, nbatch_fa,
                                         use_cp_async, oob_check, use_indirect>
...
#else
    GGML_UNUSED_VARS(Q_f2, K_h2, V_h2, mask_h, implicit_causal,
        dstk, dstk_fixup,
...
template<int DKQ, int DV, int ncols1, int ncols2, int nwarps,
         bool use_logit_softcap, bool V_is_K_view, bool needs_fixup,
         bool is_fixup, bool use_indirect = false>
...
#pragma unroll
    ...
    if constexpr (nstages > 1) {
        ...
        const half2 * const K_tile = use_indirect
            ? K_h2
            : K_h2 + int64_t(kb0)*nbatch_fa*stride_K;
        flash_attn_ext_f16_load_tile<stride_tile_K, nwarps, nbatch_fa,
                                     use_cp_async, oob_check, use_indirect>
            (K_tile, tile_K, nbatch_K2, stride_K, k_VKQ_sup,
    ...
    if constexpr (ncols2 == 1) {
        ...
        for (; kb0 < kb0_stop-1; ++kb0) {
            ...
            flash_attn_ext_f16_iter
                <DKQ, DV, ncols1, ncols2, nwarps, use_logit_softcap, V_is_K_view, use_indirect,
                 needs_fixup, is_fixup, last_iter, oob_check,
...
static __global__ void flash_attn_ext_f16(
    ...
    const bool implicit_causal = mask == nullptr && KV_max != nullptr;
    ...
    while (kbc < kbc_stop && kb0_stop == iter_k) {
        ...
        const half   * mask_h = mask == nullptr ? nullptr :
