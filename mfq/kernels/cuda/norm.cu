    ...
    double weight_offset)
{
    ...
    TORCH_CHECK(D > 0 && weight.numel() == D,
                "qwen_rms_norm_bf16: weight length mismatch");
    auto squared = torch::empty(input.sizes(), input.options().dtype(torch::kFloat32));
    ...
    double eps, double weight_offset)
{
    TORCH_CHECK(first.is_cuda() && first.is_contiguous() &&
                    ...
                    second.scalar_type() == torch::kBFloat16,
                "paired Qwen RMSNorm inputs must be cuda contiguous bf16");
    TORCH_CHECK(first_weight.is_cuda() && first_weight.is_contiguous() &&
    ...
    TORCH_CHECK(D > 0 && second.size(-1) == D &&
    ...
    auto squared = torch::empty(
        {total_rows, D}, first.options().dtype(torch::kFloat32));
    ...
    qwen_rms_norm_pair_bf16_square_kernel<<<blocks, NORM_BD, 0, stream>>>(
        reinterpret_cast<const __nv_bfloat16*>(first.data_ptr<at::BFloat16>()),
        reinterpret_cast<const __nv_bfloat16*>(second.data_ptr<at::BFloat16>()),
        squared.data_ptr<float>(), first_count, total_count);
    auto mean = squared.mean(-1, true);
    auto first_output = torch::empty_like(first);
    auto second_output = torch::empty_like(second);
    qwen_rms_norm_pair_bf16_finalize_kernel<<<blocks, NORM_BD, 0, stream>>>(
        ...
        reinterpret_cast<__nv_bfloat16*>(
            first_output.data_ptr<at::BFloat16>()),
        reinterpret_cast<__nv_bfloat16*>(
            second_output.data_ptr<at::BFloat16>()),
    ...
    double eps, double weight_offset)
{
    ...
    TORCH_CHECK(q_weight.is_cuda() && k_weight.is_cuda() &&
    ...
    TORCH_CHECK(cos.is_cuda() && sin.is_cuda() && cos.is_contiguous() &&
                    sin.is_contiguous() && cos.scalar_type() == torch::kFloat32 &&
                    sin.scalar_type() == torch::kFloat32 && cos.sizes() == sin.sizes() &&
    ...
    const int64_t total_rows = total_count / D;
    auto squared = torch::empty(
        {total_rows, D}, q.options().dtype(torch::kFloat32));
    const int blocks = (int)std::min<int64_t>(
