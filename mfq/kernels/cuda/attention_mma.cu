...
{
    static MfqAttentionMmaCache cache;
...
static MfqAttentionMmaCache & mfq_attention_mma_swa_cache(
...
static mfq_tensor_backend::Tensor mfq_attention_mma_launch(
    ...
    bool sliding, int window)
{
    ...
    MFQ_RUNTIME_CHECK(cudaGetDevice(&device) == cudaSuccess, "mfq_attention_mma256: cudaGetDevice failed");
    MFQ_RUNTIME_CHECK(cudaGetDeviceProperties(&properties, device) == cudaSuccess,
                "mfq_attention_mma256: cudaGetDeviceProperties failed");
    ...
    if (!shared_limit_set[device]) {
        ...
        MFQ_RUNTIME_CHECK(status == cudaSuccess,
                    "mfq_attention_mma256: cudaFuncSetAttribute failed: ", cudaGetErrorString(status));
...
mfq_tensor_backend::Tensor mfq_attention_mma256_cuda(
    mfq_tensor_backend::Tensor q, mfq_tensor_backend::Tensor k, mfq_tensor_backend::Tensor v, double scale)
{
    MFQ_RUNTIME_CHECK(q.is_cuda() && q.is_contiguous() && q.scalar_type() == mfq_tensor_backend::kFloat32,
                "mfq_attention_mma256: q must be contiguous CUDA f32");
    MFQ_RUNTIME_CHECK(k.is_cuda() && k.is_contiguous() && k.scalar_type() == mfq_tensor_backend::kFloat16,
                "mfq_attention_mma256: k must be contiguous CUDA f16");
    MFQ_RUNTIME_CHECK(v.is_cuda() && v.is_contiguous() && v.scalar_type() == mfq_tensor_backend::kFloat16,
                "mfq_attention_mma256: v must be contiguous CUDA f16");
    MFQ_RUNTIME_CHECK(q.dim() == 4 && k.dim() == 4 && v.dim() == 4, "mfq_attention_mma256: rank must be 4");
    const int B = (int)q.size(0);
    const int Hq = (int)q.size(1);
    const int T = (int)q.size(2);
    const int D = (int)q.size(3);
    ...
    if (T <= 32) return mfq_attention_mma_launch<256, 256, 32, 1>(q, k, v, scale, false, 0);
    return mfq_attention_mma_launch<256, 256, 64, 1>(q, k, v, scale, false, 0);
}

mfq_tensor_backend::Tensor mfq_attention_mma128_cuda(
    mfq_tensor_backend::Tensor q,
    mfq_tensor_backend::Tensor k,
    mfq_tensor_backend::Tensor v,
    double scale)
{
    MFQ_RUNTIME_CHECK(
        ...
        "mfq_attention_mma128: q must be contiguous CUDA f32");
    MFQ_RUNTIME_CHECK(
        k.is_cuda() && k.is_contiguous() &&
            k.scalar_type() == mfq_tensor_backend::kFloat16,
        "mfq_attention_mma128: k must be contiguous CUDA f16");
    MFQ_RUNTIME_CHECK(
        v.is_cuda() && v.is_contiguous() &&
            v.scalar_type() == mfq_tensor_backend::kFloat16,
        "mfq_attention_mma128: v must be contiguous CUDA f16");
    MFQ_RUNTIME_CHECK(
        q.dim() == 4 && k.dim() == 4 && v.dim() == 4,
        "mfq_attention_mma128: rank must be 4");
    ...
    if (T <= 16) {
        return mfq_attention_mma_launch<128, 128, 16, 1>(
            q, k, v, scale, false, 0);
    }
    if (T <= 32) {
        return mfq_attention_mma_launch<128, 128, 32, 1>(
            q, k, v, scale, false, 0);
    }
    return mfq_attention_mma_launch<128, 128, 64, 1>(
        q, k, v, scale, false, 0);
}

mfq_tensor_backend::Tensor mfq_attention_mma512_cuda(
    mfq_tensor_backend::Tensor q, mfq_tensor_backend::Tensor k, mfq_tensor_backend::Tensor v, double scale)
{
    MFQ_RUNTIME_CHECK(q.is_cuda() && q.is_contiguous() && q.scalar_type() == mfq_tensor_backend::kFloat32,
                "mfq_attention_mma512: q must be contiguous CUDA f32");
    MFQ_RUNTIME_CHECK(k.is_cuda() && k.is_contiguous() && k.scalar_type() == mfq_tensor_backend::kFloat16,
                "mfq_attention_mma512: k must be contiguous CUDA f16");
    MFQ_RUNTIME_CHECK(v.is_cuda() && v.is_contiguous() && v.scalar_type() == mfq_tensor_backend::kFloat16,
                "mfq_attention_mma512: v must be contiguous CUDA f16");
    MFQ_RUNTIME_CHECK(q.dim() == 4 && k.dim() == 4 && v.dim() == 4,
                "mfq_attention_mma512: rank must be 4");
    const int B = (int)q.size(0);
    const int Hq = (int)q.size(1);
    const int T = (int)q.size(2);
    const int D = (int)q.size(3);
    ...
    MFQ_RUNTIME_CHECK(D == 512 && k.size(3) == D && v.size(3) == D,
                "mfq_attention_mma512: head_dim must be 512");
    MFQ_RUNTIME_CHECK(Hq == 8 * Hk && k.size(0) == B && v.size(0) == B &&
                k.size(2) == T && v.size(2) == T && v.size(1) == Hk,
                "mfq_attention_mma512: requires self-attention with GQA ratio 8");
    MFQ_RUNTIME_CHECK(T % FATTN_KQ_STRIDE == 0,
                "mfq_attention_mma512: GQA path requires K length aligned to 64");
    return mfq_attention_mma_launch<512, 512, 8, 8>(q, k, v, scale, false, 0);
}
    ...
    mfq_tensor_backend::Tensor q, mfq_tensor_backend::Tensor kv, double scale)
{
    ...
    if (T == 1) {
        return mfq_attention_mma_launch<576, 512, 1, 16>(q, kv, kv, scale, false, 0);
    }
    if (T == 2) {
        return mfq_attention_mma_launch<576, 512, 2, 16>(q, kv, kv, scale, false, 0);
    }
    return mfq_attention_mma_launch<576, 512, 4, 16>(q, kv, kv, scale, false, 0);
}
...
{
    MFQ_RUNTIME_CHECK(q.is_cuda() && q.is_contiguous() && q.scalar_type() == mfq_tensor_backend::kFloat32,
                "mfq_attention_mma256_swa: q must be contiguous CUDA f32");
    MFQ_RUNTIME_CHECK(k.is_cuda() && k.is_contiguous() && k.scalar_type() == mfq_tensor_backend::kFloat16,
                "mfq_attention_mma256_swa: k must be contiguous CUDA f16");
    MFQ_RUNTIME_CHECK(v.is_cuda() && v.is_contiguous() && v.scalar_type() == mfq_tensor_backend::kFloat16,
                "mfq_attention_mma256_swa: v must be contiguous CUDA f16");
    MFQ_RUNTIME_CHECK(q.dim() == 4 && k.dim() == 4 && v.dim() == 4,
                "mfq_attention_mma256_swa: rank must be 4");
    const int B = (int)q.size(0);
    const int Hq = (int)q.size(1);
    const int T = (int)q.size(2);
    const int D = (int)q.size(3);
    ...
    MFQ_RUNTIME_CHECK(D == 256 && k.size(3) == D && v.size(3) == D,
                "mfq_attention_mma256_swa: head_dim must be 256");
    MFQ_RUNTIME_CHECK(Hq == 2 * Hk && k.size(0) == B && v.size(0) == B &&
                k.size(2) == T && v.size(2) == T && v.size(1) == Hk,
                "mfq_attention_mma256_swa: requires self-attention with GQA ratio 2");
    MFQ_RUNTIME_CHECK(window > 0 && window <= INT_MAX, "mfq_attention_mma256_swa: invalid window");

    if (T % FATTN_KQ_STRIDE == 0) {
        return mfq_attention_mma_launch<256, 256, 32, 2>(q, k, v, scale, true, (int)window);
    }
    if (T <= 8) return mfq_attention_mma_launch<256, 256, 8, 1>(q, k, v, scale, true, (int)window);
    if (T <= 16) return mfq_attention_mma_launch<256, 256, 16, 1>(q, k, v, scale, true, (int)window);
    if (T <= 32) return mfq_attention_mma_launch<256, 256, 32, 1>(q, k, v, scale, true, (int)window);
    return mfq_attention_mma_launch<256, 256, 64, 1>(q, k, v, scale, true, (int)window);
}

template<int DKQ_EXPECTED, int DV_EXPECTED, int NCOLS1, int NCOLS2,
         bool V_IS_K_VIEW = false>
static mfq_tensor_backend::Tensor mfq_attention_mma_decode_impl(
...
{
    MFQ_RUNTIME_CHECK(q.is_cuda() && q.is_contiguous() && q.scalar_type() == mfq_tensor_backend::kFloat32,
                "mfq_attention_mma_decode: q must be contiguous CUDA f32");
    MFQ_RUNTIME_CHECK(k_cache.is_cuda() && k_cache.is_contiguous() &&
                k_cache.scalar_type() == mfq_tensor_backend::kFloat16,
                "mfq_attention_mma_decode: k cache must be contiguous CUDA f16");
    ...
    MFQ_RUNTIME_CHECK(kv_max.is_cuda() && kv_max.is_contiguous() &&
                ...
                "mfq_attention_mma_decode: kv_max must be contiguous CUDA int32[B]");
    MFQ_RUNTIME_CHECK(meta.is_cuda() && meta.is_contiguous() && meta.scalar_type() == mfq_tensor_backend::kFloat32,
                "mfq_attention_mma_decode: meta must be contiguous CUDA f32");
    MFQ_RUNTIME_CHECK(q.dim() == 4 && q.size(2) == 1 && k_cache.dim() == 4 && v_cache.dim() == 4,
                "mfq_attention_mma_decode: expected q[B,Hq,1,D], cache[B,Hk,max_seq,D]");
    ...
    MFQ_RUNTIME_CHECK(status == cudaSuccess, "mfq_attention_mma_decode: cudaGetDevice failed: ",
                cudaGetErrorString(status));
    status = cudaGetDeviceProperties(&properties, device);
    MFQ_RUNTIME_CHECK(status == cudaSuccess, "mfq_attention_mma_decode: cudaGetDeviceProperties failed: ",
                cudaGetErrorString(status));
    const int cc = properties.major * 100 + properties.minor * 10;
    ...
    Kernel kernel = flash_attn_ext_f16<
        DKQ_EXPECTED, DV_EXPECTED, ncols1, ncols2, false, V_IS_K_VIEW>;
    static bool shmem_set[32] = {};
    MFQ_RUNTIME_CHECK(device >= 0 && device < 32, "mfq_attention_mma_decode: unsupported CUDA device index");
    if (!shmem_set[device]) {
        status = cudaFuncSetAttribute(
            kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)shmem);
        MFQ_RUNTIME_CHECK(status == cudaSuccess,
                    "mfq_attention_mma_decode: cudaFuncSetAttribute failed: ",
                    cudaGetErrorString(status));
        shmem_set[device] = true;
    }
    ...
    MFQ_RUNTIME_CHECK(status == cudaSuccess && max_blocks_per_sm > 0,
                "mfq_attention_mma_decode: occupancy query failed: ", cudaGetErrorString(status));
    const int resident_blocks = max_blocks_per_sm * properties.multiProcessorCount;
    ...
    MFQ_RUNTIME_CHECK((size_t)meta.numel() >= 2 * meta_float2,
                "mfq_attention_mma_decode: meta workspace too small");
    ...
    status = cudaGetLastError();
    MFQ_RUNTIME_CHECK(status == cudaSuccess, "mfq_attention_mma_decode launch failed: ",
                cudaGetErrorString(status));
    ...
    if (blocks_per_tile > 1) {
        ...
        status = cudaGetLastError();
        MFQ_RUNTIME_CHECK(status == cudaSuccess, "mfq_attention_mma_decode fixup launch failed: ",
                    cudaGetErrorString(status));
    }
    return out;
}
...
mfq_tensor_backend::Tensor mfq_attention_mma256_decode_cuda(
    ...
    mfq_tensor_backend::Tensor mask, mfq_tensor_backend::Tensor kv_max, mfq_tensor_backend::Tensor meta)
{
    return mfq_attention_mma_decode_impl<256, 256, 1, 8>(
...
mfq_tensor_backend::Tensor mfq_attention_mma512_decode_cuda(
    ...
    mfq_tensor_backend::Tensor mask, mfq_tensor_backend::Tensor kv_max, mfq_tensor_backend::Tensor meta)
{
    return mfq_attention_mma_decode_impl<512, 512, 1, 8>(
        q, k_cache, v_cache, seq_len, scale, planned_len, mask, kv_max, meta);
}
...
{
    return mfq_attention_mma_decode_impl<576, 512, 1, 16, true>(
...
mfq_tensor_backend::Tensor mfq_attention_mma256_swa_decode_cuda(
    ...
    mfq_tensor_backend::Tensor mask, mfq_tensor_backend::Tensor kv_max, mfq_tensor_backend::Tensor meta)
{
    return mfq_attention_mma_decode_impl<256, 256, 4, 2>(
        q, k_cache, v_cache, seq_len, scale, planned_len, mask, kv_max, meta);
}
