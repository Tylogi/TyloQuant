    ...
    int64_t gs, int64_t bits, mfq_tensor_backend::Tensor qx, mfq_tensor_backend::Tensor xscale, mfq_tensor_backend::Tensor xsum)
{
    ...
    if ((nint23_env == nullptr || nint23_env[0] == '1') && M >= 2 && M <= 6 &&
        ...
        if (bits == 2) quantize_x_kernel<16, 32><<<dim3(M, ng), 32, 0, stream>>>(
            reinterpret_cast<const __half*>(x.data_ptr<mfq_half>()), qx.data_ptr<int8_t>(),
            xscale.data_ptr<float>(), xsum.data_ptr<int32_t>(), M, K_real, K_pad);
        else quantize_x_kernel<24, 32><<<dim3(M, ng), 32, 0, stream>>>(
            reinterpret_cast<const __half*>(x.data_ptr<mfq_half>()), qx.data_ptr<int8_t>(),
            xscale.data_ptr<float>(), xsum.data_ptr<int32_t>(), M, K_real, K_pad);
        NintSmallMProjection weight{q_packed.data_ptr<uint8_t>(), sub_scale.data_ptr<uint8_t>(),
            sub_min.data_ptr<uint8_t>(), neuron_scale.data_ptr<float>(), neuron_min.data_ptr<float>(),
            out.data_ptr<mfq_half>(), N};
        launch_nint23_group4_small_m(weight, qx.data_ptr<int8_t>(), xscale.data_ptr<float>(),
            (int)bits, M, ng, K_pad, stream);
        MFQ_CUDA_KERNEL_LAUNCH_CHECK();
        return out;
    }
...
#define MFQ_NINT_BACKWARD_ROWS(ROWS) \
    ...
    if (rows == 1) {
        ...
        MFQ_NINT_BACKWARD_ROWS(16);
    }
#undef MFQ_NINT_BACKWARD_ROWS
}

template <int Bits, int Rows, bool Q5Exec>
__global__ void __launch_bounds__(32) nint_backward_quad_partial_kernel(
        const uint8_t * __restrict__ q_packed,
        const uint8_t * __restrict__ sub_scale,
        const uint8_t * __restrict__ sub_min,
        const float * __restrict__ neuron_scale,
    ...
    const int group_index = k0 / group_size;
    const int element = k0 - group_index * group_size;
    const int qbytes = Q5Exec ? 20 : (group_size * Bits + 7) / 8;
    const int split = static_cast<int>(blockIdx.y);
    const int output0 = split * output_tile;
    const int output_end = min(output0 + output_tile, outputs);
...
#pragma unroll
                for (int component = 0; component < 4; ++component) {
                    codes[component] = unpack_q5_gs28_exec_one(
                        packed_group, element + component);
                }
            } else {
                const int first_bit = element * Bits;
                const int first_byte = first_bit >> 3;
                const int shift = first_bit & 7;
                uint32_t packed = 0;
                constexpr int packed_bytes =
                    ((Bits & 1 ? 4 : 0) + 4 * Bits + 7) / 8;
#pragma unroll
                for (int byte = 0; byte < packed_bytes; ++byte) {
                    if (first_byte + byte < qbytes) {
                        packed |= static_cast<uint32_t>(
                            packed_group[first_byte + byte]) << (8 * byte);
                    }
                }
                packed >>= shift;
                constexpr uint32_t mask = (1u << Bits) - 1u;
#pragma unroll
                for (int component = 0; component < 4; ++component) {
                    codes[component] = static_cast<uint8_t>(
                        (packed >> (component * Bits)) & mask);
                }
            }
#pragma unroll
            for (int component = 0; component < 4; ++component) {
                weights[component] =
                    scale * static_cast<float>(codes[component]) - minimum;
            }
        }
#pragma unroll
        for (int row = 0; row < Rows; ++row) {
...
template <int Bits, int Rows, bool Q5Exec = false>
void launch_nint_backward_quad_partial(
        const mfq_tensor_backend::Tensor & q_packed,
        const mfq_tensor_backend::Tensor & sub_scale,
        const mfq_tensor_backend::Tensor & sub_min,
        const mfq_tensor_backend::Tensor & neuron_scale,
    ...
    const dim3 grid(
        ...
        static_cast<unsigned>((outputs + output_tile - 1) / output_tile));
    nint_backward_quad_partial_kernel<Bits, Rows, Q5Exec><<<
        grid, 32, 0, stream>>>(
            q_packed.data_ptr<uint8_t>(), sub_scale.data_ptr<uint8_t>(),
            sub_min.data_ptr<uint8_t>(), neuron_scale.data_ptr<float>(),
            neuron_min.data_ptr<float>(),
...
template <int Bits, bool Q5Exec = false>
void launch_nint_backward_quad_rows(
        const mfq_tensor_backend::Tensor & q_packed,
        const mfq_tensor_backend::Tensor & sub_scale,
        const mfq_tensor_backend::Tensor & sub_min,
        const mfq_tensor_backend::Tensor & neuron_scale,
        ...
        int output_tile,
        cudaStream_t stream) {
#define MFQ_NINT_BACKWARD_QUAD_ROWS(ROWS) \
    launch_nint_backward_quad_partial<Bits, ROWS, Q5Exec>( \
        q_packed, sub_scale, sub_min, neuron_scale, neuron_min, \
        output_gradient, partials, rows, outputs, width, groups, \
        group_size, output_tile, stream)
    if (rows == 1) {
        MFQ_NINT_BACKWARD_QUAD_ROWS(1);
    } else if (rows <= 2) {
        MFQ_NINT_BACKWARD_QUAD_ROWS(2);
    } else if (rows <= 4) {
        MFQ_NINT_BACKWARD_QUAD_ROWS(4);
    } else if (rows <= 8) {
        MFQ_NINT_BACKWARD_QUAD_ROWS(8);
    } else {
        MFQ_NINT_BACKWARD_QUAD_ROWS(16);
    }
#undef MFQ_NINT_BACKWARD_QUAD_ROWS
}
...
#pragma unroll
    ...
    for (int output = output0; output < output_end; ++output) {
        ...
        const float scale = valid
            ? output_scale * static_cast<float>(sub_scale[metadata])
            : 0.0f;
        const float minimum = valid
            ? output_minimum * static_cast<float>(sub_min[metadata])
            : 0.0f;
        float weights[4];
#pragma unroll
        for (int component = 0; component < 4; ++component) {
...
#define MFQ_NINT_BACKWARD_BITS(BITS) \
        ...
        if (q5_exec) {
            launch_nint_backward_quad_rows<5, true>(
                q_packed, sub_scale, sub_min, neuron_scale, neuron_min,
                output_gradient, partials, rows, outputs, width, groups,
                static_cast<int>(group_size), output_tile, stream);
        } else if (bits == 8 && group_size % 4 == 0) {
            launch_nint8_backward_rows(
                q_packed, sub_scale, sub_min, neuron_scale, neuron_min,
                output_gradient, partials, rows, outputs, width, groups,
                static_cast<int>(group_size), output_tile, stream);
        } else if (group_size % 4 == 0) {
#define MFQ_NINT_BACKWARD_QUAD_BITS(BITS) \
            launch_nint_backward_quad_rows<BITS>( \
                q_packed, sub_scale, sub_min, neuron_scale, neuron_min, \
                output_gradient, partials, rows, outputs, width, groups, \
                static_cast<int>(group_size), output_tile, stream)
            switch (static_cast<int>(bits)) {
                case 1: MFQ_NINT_BACKWARD_QUAD_BITS(1); break;
                case 2: MFQ_NINT_BACKWARD_QUAD_BITS(2); break;
                case 3: MFQ_NINT_BACKWARD_QUAD_BITS(3); break;
                case 4: MFQ_NINT_BACKWARD_QUAD_BITS(4); break;
                case 5: MFQ_NINT_BACKWARD_QUAD_BITS(5); break;
                case 6: MFQ_NINT_BACKWARD_QUAD_BITS(6); break;
                case 7: MFQ_NINT_BACKWARD_QUAD_BITS(7); break;
            }
#undef MFQ_NINT_BACKWARD_QUAD_BITS
        } else {
            switch (static_cast<int>(bits)) {
                ...
                case 8: MFQ_NINT_BACKWARD_BITS(8); break;
            }
        }
