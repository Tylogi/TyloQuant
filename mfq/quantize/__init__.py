...
- ``nint_quant_torch`` -- torch/CUDA NINT tensor-level quantization (the main large-model conversion path).
- ``nvq_quant`` -- joint search over E8/D4 codewords and neuron/sub-group scales.
- ``nvq_quant_torch`` -- offline CUDA quantization and native assignment-kernel dispatch for NVQ1-L/NVQ2/NVQ3.
...
- ``nvq1_s_quant`` -- NVQ1-S 512-entry ternary VQ and fixed 32-bit group solver.
