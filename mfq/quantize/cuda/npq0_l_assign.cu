...
constexpr int kFirstEntries = 8;
constexpr int kSecondEntries = 16;
...
__device__ __forceinline__ int code_offset(
    ...
    int entry,
    int coordinate) {
    return (state * Entries + entry) * kSubvector + coordinate;
}

__global__ void npq0_l_assign_kernel(
...
#pragma unroll
    for (int state = 0; state < kStates; ++state) {
        float first_error = FLT_MAX;
        int first_index = lane;
        if (lane < kFirstEntries) {
            float signal = 0.0f;
            float dot = 0.0f;
            float norm = 0.0f;
#pragma unroll
            for (int coordinate = 0; coordinate < kSubvector; ++coordinate) {
                if (coordinate < valid_first) {
                    const float x = shared_value[vector_position + coordinate];
                    const float w = shared_weight[vector_position + coordinate];
                    const float code = static_cast<float>(
                        first_codebooks[
                            code_offset<kFirstEntries>(state, lane, coordinate)]);
            ...
            first_error = fmaf(
                scale * scale, norm, fmaf(-2.0f * scale, dot, signal));
        }
        for (int shift = 4; shift > 0; shift >>= 1) {
            const float other_error =
                __shfl_down_sync(0xffffffff, first_error, shift, 8);
            const int other_index =
                __shfl_down_sync(0xffffffff, first_index, shift, 8);
            if (lane < shift
                    && better(
                        other_error,
                        other_index,
        ...
        float second_error = FLT_MAX;
        int second_index = lane;
        if (lane < kSecondEntries) {
            float signal = 0.0f;
            float dot = 0.0f;
            float norm = 0.0f;
#pragma unroll
            for (int coordinate = 0; coordinate < kSubvector; ++coordinate) {
                if (coordinate < valid_second) {
                    const int position = vector_position + kSubvector + coordinate;
                    const float x = shared_value[position];
                    const float w = shared_weight[position];
                    const float code = static_cast<float>(
                        second_codebooks[
                            code_offset<kSecondEntries>(state, lane, coordinate)]);
                    signal = fmaf(w * x, x, signal);
                    dot = fmaf(w * x, code, dot);
                    norm = fmaf(w * code, code, norm);
                }
        ...
        for (int shift = 8; shift > 0; shift >>= 1) {
            const float other_error =
                __shfl_down_sync(0xffffffff, second_error, shift, 16);
            const int other_index =
                __shfl_down_sync(0xffffffff, second_index, shift, 16);
            if (lane < shift
                    && better(
                        other_error,
                        other_index,
        ...
        if (lane == 0) {
            vector_error[state][warp] = first_error + second_error;
            vector_first[state][warp] = static_cast<uint8_t>(first_index);
...
__global__ void npq0_l_refit_anchor_kernel(
    const float* __restrict__ value,
    const float* __restrict__ objective_weight,
    ...
    for (int position = tid; position < valid_width; position += blockDim.x) {
        ...
        const int entry = second
            ...
            : first_indices[
                static_cast<int64_t>(flat_group) * kVectorsPerGroup + vector];
        const float code = second
            ? static_cast<float>(second_codebooks[
                code_offset<kSecondEntries>(state, entry, coordinate)])
            : static_cast<float>(first_codebooks[
                code_offset<kFirstEntries>(state, entry, coordinate)]);
...
void check_inputs(
    ...
    TORCH_CHECK(
        ...
        "npq0_l_assign: value must be CUDA contiguous float32 [rows,padded_K]");
    TORCH_CHECK(
        ...
        "npq0_l_assign: objective_weight must match value");
    TORCH_CHECK(
    ...
    TORCH_CHECK(
        ...
        "npq0_l_assign: tensors must share one CUDA device");
}
...
std::vector<torch::Tensor> npq0_l_assign_cuda(
    ...
    auto launch_assign = [&](const torch::Tensor& current_anchor) {
        npq0_l_assign_kernel<<<groups, kAssignThreads, 0, stream>>>(
    ...
    auto launch_refit = [&](
        ...
        npq0_l_refit_anchor_kernel<<<rows, kRefitThreads, 0, stream>>>(
            value.data_ptr<float>(),
            objective_weight.data_ptr<float>(),
            current_anchor.data_ptr<float>(),
            scale_lut.data_ptr<float>(),
