"""Weighted-SSE trainer and fixed-table quantizer for NPQ0-L."""
...
from mfq.formats.npq0_l import Npq0LTensor, validate_npq0_l_tables
...
class Npq0LConfig:
    ...
    def __post_init__(self) -> None:
        if self.iterations < 0 or self.assignment_refine_steps < 0:
            raise ValueError("NPQ0-L iteration counts must be non-negative")
        if self.fixed_refine_steps < 0 or self.kmeans_iterations < 0:
            raise ValueError("NPQ0-L refinement counts must be non-negative")
        if self.kmeans_initialization_points <= 0 or self.group_chunk <= 0:
            raise ValueError("NPQ0-L chunk sizes must be positive")
        if not self.anchor_multipliers or any(
            not math.isfinite(value) or value <= 0 for value in self.anchor_multipliers
        ):
            raise ValueError("NPQ0-L anchor multipliers must be finite and positive")
...
def npq0_l_tables_from_tensor(tensor: Npq0LTensor) -> Npq0LTables:
    return Npq0LTables(
    ...
    *,
    ...
    for start in range(0, groups, group_chunk):
        ...
        for state_id in range(_STATE_COUNT):
            ...
            error = (first_error + second_error).reshape(count, _VECTORS_PER_GROUP).sum(1)
            ...
            local_first[better] = first_index.reshape(count, _VECTORS_PER_GROUP)[better]
            local_second[better] = second_index.reshape(count, _VECTORS_PER_GROUP)[better]
...
def _weighted_kmeans(
    ...
    *,
    ...
    if not samples.shape[0]:
        return torch.zeros((entries, _SUBVECTOR_SIZE), dtype=torch.int8, device=samples.device)
    ...
    for _ in range(iterations):
        numerator = torch.zeros((entries, 4), device=samples.device)
        ...
        for start in range(0, samples.shape[0], 8192):
            ...
            expanded = index.unsqueeze(1).expand(-1, 4)
            numerator.scatter_add_(
                0,
                expanded,
...
def _initialize(
    ...
    *,
    ...
    weights = wgroup.reshape_as(vectors)
    first = torch.empty(
        (_STATE_COUNT, _FIRST_ENTRIES, 4),
        dtype=torch.int8,
        device=xgroup.device,
    )
    second = torch.empty(
        (_STATE_COUNT, _SECOND_ENTRIES, 4),
        dtype=torch.int8,
        device=xgroup.device,
    )
...
def train_npq0_l(
    ...
    *,
    ...
    config: Npq0LConfig | None = None,
    device: str | torch.device = "cuda",
) -> tuple[Npq0LTensor, tuple[Npq0LIteration, ...]]:
    """Train one tensor-wise mixture-PQ table set by coordinate descent."""

    config = Npq0LConfig() if config is None else config
    if not torch.cuda.is_available() and str(device).startswith("cuda"):
        raise RuntimeError("NPQ0-L CUDA training requested without a CUDA device")
    value, out, neuron_len = _prepare_weight(weight, device)
    if neuron_len % _VECTOR_SIZE:
        raise ValueError("NPQ0-L requires K divisible by 8")
...
def quantize_npq0_l_fixed(
    ...
    *,
    ...
    config = Npq0LConfig() if config is None else config
    if not torch.cuda.is_available() and str(device).startswith("cuda"):
        raise RuntimeError("NPQ0-L CUDA quantization requested without a CUDA device")
...
def dequantize_npq0_l(tensor: Npq0LTensor) -> np.ndarray:
    out = int(np.asarray(tensor.neuron_scale).size)
    neuron_len = int(tensor.neuron_len)
    ng = math.ceil(neuron_len / _GROUP_SIZE)
    nvec = neuron_len // _VECTOR_SIZE
    padded_nvec = ng * _VECTORS_PER_GROUP
    composite = np.zeros((out, padded_nvec), dtype=np.uint8)
    composite[:, :nvec] = np.asarray(tensor.indices, dtype=np.uint8)
    first_index = composite & 7
    second_index = composite >> 3
    vector_state = np.repeat(np.asarray(tensor.state, dtype=np.uint8), 3, axis=1)
    first = np.asarray(tensor.first_codebooks, dtype=np.int8)[vector_state, first_index]
    second = np.asarray(tensor.second_codebooks, dtype=np.int8)[vector_state, second_index]
    ...
    matrix = code.astype(np.float32) * scale
