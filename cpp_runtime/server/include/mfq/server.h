...
struct MfqMultimodalInput {
    ...
    // Stable media-processor contract from the model manifest. The server
    // transports this identity but never branches on an architecture name.
...
// Runtime capabilities come from model_graph.json. Architecture-name lookup
// remains only as a compatibility fallback for pre-schema callers.
