...
std::string deepseek_hf_expert_name(std::string_view canonical) {
    ...
    if (canonical.starts_with(model_prefix)) {
        result = "layers." + std::string(canonical.substr(model_prefix.size()));
    } else if (canonical.starts_with(predictor_prefix)) {
        result = "mtp." + std::string(canonical.substr(predictor_prefix.size()));
    }
    const auto experts = result.find(".mlp.experts.");
    if (experts == std::string::npos) {
        throw std::invalid_argument("invalid DeepSeek HF expert tensor name");
    }
    result.replace(experts, std::string_view(".mlp.experts.").size(),
                   ".ffn.experts.");
    constexpr std::array<std::pair<std::string_view, std::string_view>, 6>
        suffixes{{
            {".gate.weight_scale", ".w1.scale"},
            {".down.weight_scale", ".w2.scale"},
            {".up.weight_scale", ".w3.scale"},
            {".gate.weight", ".w1.weight"},
            {".down.weight", ".w2.weight"},
            {".up.weight", ".w3.weight"},
        }};
    for (const auto& [source, target] : suffixes) {
        if (result.ends_with(source)) {
            result.replace(result.size() - source.size(), source.size(), target);
            return result;
        }
    }
    throw std::invalid_argument("invalid DeepSeek HF expert projection");
}
...
struct MlxDeepseekV4SsdExpertCache::Impl {
    ...
    Impl(
        ...
        : store(
              std::make_shared<HfSafetensorStore>(std::move(root)),
              std::move(layer_prefixes),
              num_experts,
              hidden_size,
              intermediate_size,
              deepseek_hf_expert_name),
