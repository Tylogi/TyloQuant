...
constexpr const char *kSq2BackwardInput = R"METAL(
    uint lane = thread_index_in_simdgroup;
    ...
    for (uint output = lane; output < uint(OUT); output += 32u) {
        uint block_index = output * uint(BLOCKS) + block;
        uint tag = mfq_sq2_scalar_block_tag(symbols, selectors, block_index);
        uint state_index = output * uint(STATES) + tag;
        float scale = mfq_sq2_read_scale(state_scales, state_index);
        uint palette = mfq_sq2_read_palette(state_palettes, state_index);
        uint symbol = mfq_sq2_read_symbol(symbols, block_index, component);
        float weight = mfq_sq2_palette_values[palette * 4u + symbol] * scale;
        for (uint local = 0u; local < uint(TILE_M); ++local) {
...
const mlx::core::fast::CustomKernelFunction &sq2_backward_input_kernel() {
  static const auto kernel = make_sq2_matmul_kernel(
      "mfq_cpp_mxfp4_sq2_backward_input", kSq2BackwardInput);
  return kernel;
}
...
array MlxMxfp4Sq2Weight::backward_input(
  ...
  auto arguments = mmq_templates(
      ...
      static_cast<int>(rows), tile_rows);
  auto outputs = sq2_backward_input_kernel()(
