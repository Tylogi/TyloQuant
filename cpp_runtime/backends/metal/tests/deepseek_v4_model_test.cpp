...
void test_required_ratio_schedule() {
    ...
    require(
        !contains(
            required,
            "model.block.0.attention.compressor.key_value.weight"),
        "ratio-0 layer incorrectly requires a compressor");
    require(
        contains(
            required,
            "model.block.1.attention.compressor.key_value.weight") &&
            contains(
                required,
                "model.block.1.attention.indexer.query.weight") &&
            contains(
                required,
                "model.block.1.attention.indexer.compressor.position"),
    ...
    require(
        contains(
            ...
            !contains(
                required,
                "model.block.2.attention.indexer.query.weight"),
        "ratio-128 layer has the wrong Indexer schedule");
    require(
        contains(
            required,
            "model.block.0.mlp.router.token_to_expert") &&
            contains(
                required,
                "model.block.1.mlp.router.bias"),
...
void test_container_bindings(
    ...
    require(
        embedding.dtype == "F16" &&
            ...
            !embedding.packed,
        "canonical embedding metadata mismatch");
    ...
    write_model(
        ...
        "deepseek_v4-hf-mfq-nint-recipe",
        {},
        "model.block.2.attention.compressor.position");
    ...
    require_throws(
        ...
        "requires a deepseek_v4 artifact",
