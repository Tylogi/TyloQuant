...
Fixture make_fixture(int rows, int columns) {
  require(rows > 0 && columns > 0 && columns % 32 == 0,
          "invalid SQ3 test fixture geometry");
  ...
  for (int row = 0; row < rows; ++row) {
    ...
    for (int block = 0; block < blocks_per_row; ++block) {
      ...
      low_tag ^= symbols[last_index] & 3u;
      require(low_tag == required_low_tag,
              "failed to construct SQ3 implicit test tag");
      ...
      for (int lane = 0; lane < 32; ++lane) {
        ...
        const auto nibble = mfq::metal::kMxfp4Sq3PaletteNibbles
            [static_cast<std::size_t>(palette) * 8 + symbols[index]];
  ...
  if (blocks >= seen_tags.size()) {
    for (bool seen : seen_tags) {
      require(seen, "SQ3 test fixture did not cover every block tag");
    }
  }

  const auto packed_symbols = pack_bits(symbols, 3);
  ...
  std::vector<std::uint8_t> blob{'S', 'Q', '3', '1'};
...
void test_dequantize() {
  ...
  require(weight.matrix_scale_base() == fixture.matrix_scale_base,
          "SQ3 scale base mismatch");
  require(weight.packed_nbytes() == fixture.payload_nbytes,
          "SQ3 payload byte count mismatch");
  ...
  for (std::size_t index = 0; index < fixture.dense.size(); ++index) {
    require(fp32.data<float>()[index] == fixture.dense[index],
            "SQ3 FP32 dequantization mismatch");
    require(fp16.data<float>()[index] == fixture.dense[index],
            "SQ3 FP16 dequantization mismatch");
  }
}
...
void test_fused_gemv() {
  ...
  require(output_fp16.shape() == Shape{1, fixture.rows},
          "SQ3 fused GEMV shape mismatch");
  require(output_fp32.dtype() == float32, "SQ3 FP32 GEMV dtype mismatch");
  for (int row = 0; row < fixture.rows; ++row) {
    ...
    require(std::fabs(output_fp16.data<float>()[row] - expected) < 2e-3f,
            "SQ3 FP16 fused GEMV value mismatch");
    require(std::fabs(output_fp32.data<float>()[row] - expected) < 2e-3f,
            "SQ3 FP32 fused GEMV value mismatch");
  }
}
...
void test_multirow_buckets() {
  ...
  for (const int rows : row_counts) {
    ...
    require(output.shape() == expected_shape,
            "SQ3 multirow bucket shape mismatch at M=" +
                std::to_string(rows));
    ...
    require(maximum_difference < 8e-3f,
            "SQ3 multirow bucket value mismatch at M=" +
...
void test_fp32_multirow_contract() {
  ...
  require(actual.dtype() == float32, "SQ3 FP32 multirow dtype mismatch");
  for (std::size_t index = 0; index < actual.size(); ++index) {
    require(actual.data<float>()[index] == expected.data<float>()[index],
            "SQ3 FP32 multirow contract mismatch");
  }
}
...
void test_blob_validation() {
  ...
  truncated.pop_back();
  require_rejected(truncated, "SQ3 truncated payload was accepted");

  auto invalid_magic = fixture.blob;
  invalid_magic[0] = 'X';
  require_rejected(invalid_magic, "SQ3 invalid magic was accepted");

  auto trailing = fixture.blob;
  trailing.push_back(0);
  require_rejected(trailing, "SQ3 trailing payload was accepted");

  auto invalid_scale = fixture.blob;
  invalid_scale[5] = 252;
  require_rejected(invalid_scale, "SQ3 invalid E8M0 base was accepted");
}
