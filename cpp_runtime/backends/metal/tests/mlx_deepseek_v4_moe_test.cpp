...
streamed_model_records(
    ...
    const auto layer =
        [](std::string_view suffix) {
            return std::string("model.block.0.")
                + std::string(suffix);
        };
    return {
        {
        ...
        {
            layer("mlp.router.token_to_expert"),
            "I32",
            dense_payload(
                token_experts,
                {kVocab, kTopK}),
...
void test_named_dspark_moe_reuses_streamed_tpq_cache() {
    ...
    for (auto& record : records) {
        if (record.name.starts_with("model.block.0.")) {
            record.name.replace(0, 14, "predictor.stage.0.");
        }
    }
    const TemporaryDeepseekMfq file(
        records,
