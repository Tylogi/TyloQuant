...
void test_host_sampling_distribution() {
    ...
    const mlx::core::array logits(
        values.begin(), mlx::core::Shape{1, 3}, mlx::core::float32);
    mfq::metal::MlxSamplingParams params;
    params.temperature = 1.0;
    params.top_k = 2;
    params.top_p = 1.0;
    const auto probabilities =
        mfq::metal::host_sampling_distribution(logits, params);
    require(probabilities.size() == 3, "host distribution size mismatch");
    require(std::abs(probabilities[0]) < 1e-6f,
            "host top-k retained an excluded token");
    require(std::abs(probabilities[1] - 0.2689414f) < 1e-5f &&
                std::abs(probabilities[2] - 0.7310586f) < 1e-5f,
            "host top-k probabilities mismatch");
    params.top_p = 0.7;
    const auto nucleus =
        mfq::metal::host_sampling_distribution(logits, params);
    require(nucleus[2] == 1.0f && nucleus[0] == 0.0f &&
                nucleus[1] == 0.0f,
