...
void test_embedding_forward_parity_and_hidden() {
    const auto config = test_config();
    const auto ids = token_ids({1, 4, 2});
    auto embedding = make_embedding(config);
    const auto embeddings = embedding(ids, mlx::core::float32);

    auto token_model = make_model();
    auto embedding_model = make_model();
    auto hidden_model = make_model();
    auto token_logits = token_model.forward(ids, false);
    auto embedding_logits = embedding_model.forward_embeddings(
        embeddings,
        false);
    auto [hidden_logits, hidden] =
        hidden_model.forward_embeddings_with_hidden(
            embeddings,
            false);

    require_array_close(
        std::move(embedding_logits),
        token_logits,
        2.5e-3f,
        "token versus embedding forward");
    require_array_close(
        std::move(hidden_logits),
        std::move(token_logits),
        2.5e-3f,
        "embedding forward-with-hidden logits");

    auto expected_hidden = embeddings;
    auto linear = make_linear_block(config);
    auto full = make_full_block(config);
    expected_hidden = linear.forward(expected_hidden, 0, false);
    expected_hidden = full.forward(expected_hidden, 0, false);
    require_array_close(
        std::move(hidden),
        std::move(expected_hidden),
        2.5e-3f,
        "embedding forward hidden states");

    const auto positions = three_axis_positions(
        {0, 1, 2},
        {0, 13, 31},
        {0, 7, 29});
    auto positioned_token_model = make_model();
    auto positioned_embedding_model = make_model();
    auto positioned_hidden_model = make_model();
    auto positioned_token_logits = positioned_token_model.forward(
        ids,
        positions,
        false);
    auto positioned_embedding_logits =
        positioned_embedding_model.forward_embeddings(
            embeddings,
            positions,
            false);
    auto positioned_result =
        positioned_hidden_model.forward_embeddings_with_hidden(
            embeddings,
            positions,
            false);
    require_array_close(
        std::move(positioned_embedding_logits),
        positioned_token_logits,
        2.5e-3f,
        "MRoPE token versus embedding forward");
    require_array_close(
        std::move(positioned_result.first),
        std::move(positioned_token_logits),
        2.5e-3f,
        "MRoPE embedding forward-with-hidden logits");

    const array batch_ids(
        {1, 4, 2, 5},
        Shape{2, 2},
        mlx::core::int32);
    auto batch_embeddings = embedding(
        batch_ids,
        mlx::core::float32);
    auto batch_model = make_model();
    auto batch_logits = batch_model.forward_embeddings(
        batch_embeddings,
        true);
    batch_logits.eval();
    require(
        batch_logits.shape() == Shape({2, 2, 8}) &&
            batch_model.cache_batch() == 2 &&
            batch_model.cache_position() == 2,
        "batched embedding forward shape/cache mismatch");
}

void test_embedding_cache_continuity() {
    const auto config = test_config();
    auto embedding = make_embedding(config);
    const auto all_ids = token_ids({2, 5, 3});
    const auto all_embeddings = embedding(
        all_ids,
        mlx::core::float32);

    auto full_text_model = make_model();
    auto full_text_logits = full_text_model.forward_embeddings(
        all_embeddings,
        true);
    auto chunked_text_model = make_model();
    auto prefix_embeddings = mlx::core::slice(
        all_embeddings,
        Shape{0, 0, 0},
        Shape{1, 2, 4});
    auto final_embedding = mlx::core::slice(
        all_embeddings,
        Shape{0, 2, 0},
        Shape{1, 3, 4});
    auto prefix_logits = chunked_text_model.forward_embeddings(
        prefix_embeddings,
        true);
    auto decoded_logits = chunked_text_model.forward_embeddings(
        final_embedding,
        true);
    mlx::core::eval(
        full_text_logits,
        ...
        decoded_logits);
    require_array_close(
        std::move(decoded_logits),
        last_logits(full_text_logits),
        3.0e-3f,
        "contiguous embedding prefill/decode");
    require(
        full_text_model.cache_position() == 3 &&
            chunked_text_model.cache_position() == 3,
        "contiguous embedding cache did not advance by token count");

    const auto all_positions = three_axis_positions(
        {0, 1, 2},
        {0, 13, 31},
        {0, 7, 29});
    auto full_mrope_model = make_model();
    auto full_mrope_logits = full_mrope_model.forward(
        all_ids,
        all_positions,
        true);
    auto mixed_model = make_model();
    auto mixed_prefix = mixed_model.forward_embeddings(
        prefix_embeddings,
        three_axis_positions(
            {0, 1},
            {0, 13},
            {0, 7}),
        true);
    auto mixed_decode = mixed_model.forward(
        token_ids({3}),
        three_axis_positions(
            {2},
            {31},
    ...
    mlx::core::eval(
        ...
        mixed_decode);
    require_array_close(
        std::move(mixed_decode),
        last_logits(full_mrope_logits),
        3.0e-3f,
        "embedding/token shared MRoPE cache");
    require(
        mixed_model.cache_batch() == 1 &&
            mixed_model.cache_position() == 3,
        "embedding/token shared cache metadata mismatch");
}

void test_embedding_validation() {
    const auto rejected = [](array embeddings) {
        auto model = make_model();
        try {
            (void)model.forward_embeddings(embeddings, false);
        } catch (const std::runtime_error&) {
            return true;
        }
        return false;
    ...
    require(
        rejected(mlx::core::zeros(Shape{2, 4}, mlx::core::float32)),
        "rank-two embeddings were accepted");
    require(
        rejected(mlx::core::zeros(Shape{1, 0, 4}, mlx::core::float32)),
        "empty embeddings were accepted");
    require(
        rejected(mlx::core::zeros(Shape{1, 2, 3}, mlx::core::float32)),
        "wrong-width embeddings were accepted");
    require(
        rejected(mlx::core::zeros(Shape{1, 2, 4}, mlx::core::int32)),
        "integer embeddings were accepted");

    auto model = make_model();
    bool position_mismatch_rejected = false;
    try {
        (void)model.forward_embeddings(
            mlx::core::zeros(Shape{1, 2, 4}, mlx::core::float32),
            text_positions(0, 3),
            false);
    } catch (const std::runtime_error&) {
        position_mismatch_rejected = true;
    }
    require(
        position_mismatch_rejected,
        "embedding/position token-count mismatch was accepted");

    auto cache_model = make_model();
    auto first = cache_model.forward_embeddings(
        mlx::core::zeros(Shape{1, 1, 4}, mlx::core::float32),
        true);
    first.eval();
    bool cache_batch_mismatch_rejected = false;
    try {
        (void)cache_model.forward_embeddings(
            mlx::core::zeros(Shape{2, 1, 4}, mlx::core::float32),
            true);
    } catch (const std::runtime_error&) {
        cache_batch_mismatch_rejected = true;
    }
    require(
        cache_batch_mismatch_rejected &&
            cache_model.cache_batch() == 1 &&
            cache_model.cache_position() == 1,
        "embedding cache batch mismatch was not rejected cleanly");
}
...
void test_mtp_greedy_identity() {
    ...
    penalized.repetition_penalty = 1.2;
    auto penalized_baseline = make_model();
    auto penalized_speculative = make_model(false, false, true);
    std::vector<std::int64_t> penalized_expected;
    std::vector<std::int64_t> penalized_actual;
    const auto penalized_expected_count = penalized_baseline.generate(
        prompt,
        penalized,
        8,
        [&](std::int64_t token) {
            penalized_expected.push_back(token);
            return true;
        });
    const auto penalized_actual_count = penalized_speculative.generate(
        prompt,
        penalized,
        8,
        [&](std::int64_t token) {
            penalized_actual.push_back(token);
            return true;
        });
    const auto& penalized_stats =
        penalized_speculative.last_mtp_stats();
    require(
        penalized_expected_count == 8 &&
            penalized_actual_count == 8 &&
            penalized_actual == penalized_expected &&
            penalized_stats.used &&
            penalized_stats.cycles > 0,
        "sampling penalties disabled MTP or changed target generation");
    ...
    auto sampled_speculative = make_model(false, false, true);
    auto sampled_repeated = make_model(false, false, true);
    std::vector<std::int64_t> actual_sampled;
    std::vector<std::int64_t> repeated_sampled;
    const auto sampled_count = sampled_speculative.generate(
        prompt,
        stochastic,
        8,
        [&](std::int64_t token) {
    ...
    const auto repeated_count = sampled_repeated.generate(
        prompt,
        stochastic,
        8,
        [&](std::int64_t token) {
            repeated_sampled.push_back(token);
            return true;
        });
    require(
        sampled_count == 8 && repeated_count == 8 &&
            actual_sampled == repeated_sampled &&
            std::all_of(
                actual_sampled.begin(), actual_sampled.end(),
                [](std::int64_t token) { return token >= 0 && token < 8; }),
...
void test_mtp_prepared_mrope_generation() {
    ...
    const auto ids = token_ids({1, 4, 2});
    const std::vector<std::int32_t> position_values{
        0, 1, 2,
        0, 1, 2,
        0, 1, 2,
    };
    mfq::metal::MlxPreparedPrompt prepared;
    prepared.token_ids = {1, 4, 2};
    prepared.embeddings = model.embed_tokens(ids);
    prepared.positions.emplace(
        position_values.begin(), mlx::core::Shape{3, 3}, mlx::core::int32);
    ...
    const auto count = model.generate_prepared(
        prepared,
        sampling,
        6,
        [&](std::int64_t token) {
    ...
    require(
        count == 6 && output.size() == 6 && model.supports_mtp(),
    ...
    try {
        ...
        if (argc == 2) {
            ...
            auto mtp = MlxQwen35MtpModule::load_if_present(
                container,
                config,
                mfq::metal::Qwen35TensorNames::canonical());
        ...
        test_embedding_forward_parity_and_hidden();
