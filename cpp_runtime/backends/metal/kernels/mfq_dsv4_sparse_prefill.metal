...
// Direct local/pool sparse-attention geometry.  This follows the execution
...
void mfq_dsv4_sparse_circular(
    ...
    // q is [B,H,M,D], local/pool are [B,L,D], topk is [B,M,K], and
    // output is [B,M,H,D].  All inputs are made contiguous by the C++ seam.
    const device T* q_base = q
        + (size_t(batch) * H * params.queries + query_position) * D;
    const device T* local_base = local_kv
        + size_t(batch) * params.local_length * D;
    const device T* pooled_base = pooled_kv
        + size_t(batch) * params.pool_capacity * D;
    const device int* topk_base = topk
        + (size_t(batch) * params.queries + query_position) * params.topk;

    const int local_offset = params.local_length - params.queries;
    const int local_end = metal::min(
        params.local_length,
        local_offset + query_position + 1);
    const int local_start = metal::max(
        0,
        local_end - params.local_window);
    ...
    const int pooled_valid = metal::min(
        params.pool_length,
        (params.query_offset + query_position + 1) / params.pool_ratio);
    const int total_tiles = local_tiles + pooled_tiles;

    for (int key_tile = 0; key_tile < total_tiles; ++key_tile) {
        const bool is_pooled = key_tile >= local_tiles;
        const int tile_index = is_pooled
            ? key_tile - local_tiles
            : key_tile;
        const int slot_base = tile_index * BK;
        for (int k = lane; k < BK; k += tgp_size) {
            const int slot = slot_base + k;
            int row = -1;
            if (is_pooled) {
                if (slot < params.topk) {
                    const int pooled_row = topk_base[slot];
                    if (pooled_row >= 0 && pooled_row < pooled_valid) {
                        row = pooled_row;
                    }
                }
            } else if (slot < local_count) {
                row = local_start + slot;
            }
            selected_rows[k] = row;
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        ...
        for (short chunk = 0; chunk < D_CHUNKS; ++chunk) {
            ...
            for (int element = lane; element < BK * DC; element += tgp_size) {
                ...
                T value = T(0);
                if (row >= 0) {
                    const device T* cache = is_pooled
                        ? pooled_base
                        : local_base;
                    value = cache[
                        size_t(row) * D + dimension_base + dimension];
                }
                KVs[k + dimension * LDK] = value;
            }
            threadgroup_barrier(mem_flags::mem_threadgroup);
            STEEL_PRAGMA_UNROLL
            for (short block = 0; block < TDC; ++block) {
