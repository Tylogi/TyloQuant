...
MlxDeepseekV4DSparkAttentionComponents load_attention(
    ...
    return {
        ...
        MlxLinear::load(model, name("key_value_a.weight")),
        MlxLinear::load(model, name("output_a.weight")),
        MlxLinear::load(model, name("output_b.weight")),
        load_float(model, name("query_a_norm.weight")),
        load_float(model, name("key_value_a_norm.weight")),
        load_float(model, name("sink")),
    };
}
...
MlxDeepseekV4DSparkStageComponents load_stage(
    ...
    return {
        ...
        load_float(model, name("attention.mhc.pre.base")),
        load_float(model, name("attention.mhc.pre.scale")),
...
MlxDeepseekV4DSpark MlxDeepseekV4DSpark::load_hf(
    ...
    return MlxDeepseekV4DSpark(
        ...
        std::move(stages),
        {
            ...
            model.load_linear(last + ".mhc.output.function"),
            float32_contiguous(model.load_dense(last + ".mhc.output.base")),
            float32_contiguous(model.load_dense(last + ".mhc.output.scale")),
...
MlxDeepseekV4DSparkDraft MlxDeepseekV4DSpark::draft(
    ...
    // DSpark attention over its draft block is intentionally non-causal.
    // Preserve the trained fixed-width noise geometry, then truncate only
    // the returned heads when the adaptive controller asks for fewer drafts.
