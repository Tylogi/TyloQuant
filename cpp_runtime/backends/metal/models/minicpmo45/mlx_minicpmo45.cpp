...
public:
    static MiniQwen3Ffn load(
        ...
        const std::string& prefix) {
        return MiniQwen3Ffn(
            MiniLinear::load(model, prefix + ".gate", false),
            MiniLinear::load(model, prefix + ".up", false),
            MiniLinear::load(model, prefix + ".down", false));
    }
    ...
    static MiniQwen3Block load(
        ...
        if (config.query_key_norm) {
            query_norm = mini_rms_norm(
                model,
                layer + ".attention.query_norm.weight",
                config.norm_eps);
            key_norm = mini_rms_norm(
                model,
                layer + ".attention.key_norm.weight",
                config.norm_eps);
        }
        return MiniQwen3Block(
            config,
            mini_rms_norm(
                model, layer + ".attention.norm.weight", config.norm_eps),
            MiniLinear::load(model, layer + ".attention.query", false),
            MiniLinear::load(model, layer + ".attention.key", false),
            MiniLinear::load(model, layer + ".attention.value", false),
            MiniLinear::load(model, layer + ".attention.output", false),
            std::move(query_norm),
            std::move(key_norm),
            mini_rms_norm(
                model,
                layer + ".mlp.norm.weight",
                config.norm_eps),
    ...
    static VisionAttention load(
        ...
        if (fuse_qkv) {
            auto weight = mlx::core::concatenate(
                {
                    dense(model, prefix + ".query.weight"),
                    dense(model, prefix + ".key.weight"),
                    dense(model, prefix + ".value.weight"),
                },
                0);
            auto bias = mlx::core::concatenate(
                {
                    dense(model, prefix + ".query.bias"),
                    dense(model, prefix + ".key.bias"),
                    dense(model, prefix + ".value.bias"),
                },
                0);
            return VisionAttention(
                std::move(weight),
                std::move(bias),
                MiniLinear::load(model, prefix + ".output"));
        }
        return VisionAttention(
            MiniLinear::load(model, prefix + ".query"),
            MiniLinear::load(model, prefix + ".key"),
            MiniLinear::load(model, prefix + ".value"),
            MiniLinear::load(model, prefix + ".output"));
    }
...
public:
    static VisionLayer load(
        ...
        return VisionLayer(
            VisionAttention::load(model, prefix + ".attention"),
            MiniLayerNorm::load(model, prefix + ".norm1", 1e-6f),
            MiniLayerNorm::load(model, prefix + ".norm2", 1e-6f),
            MiniLinear::load(model, prefix + ".mlp.up"),
            MiniLinear::load(model, prefix + ".mlp.down"));
    }
...
public:
    static VisionEncoder load(const MfqContainer& model) {
        ...
        return VisionEncoder(
            dense(model, "vision.patch_embedding.weight"),
            dense(model, "vision.patch_embedding.bias"),
            dense(model, "vision.position_embedding.weight"),
            std::move(layers),
            MiniLayerNorm::load(model, "vision.output_norm", 1e-6f));
    }
...
public:
    static Resampler load(const MfqContainer& model) {
        return Resampler(
            dense(model, "vision.resampler.query"),
            load_resampler_position(model),
            MiniLinear::load(model, "vision.resampler.key_value", false),
            MiniLayerNorm::load(model, "vision.resampler.query_norm", 1e-6f),
            MiniLayerNorm::load(model, "vision.resampler.key_value_norm", 1e-6f),
            MiniLayerNorm::load(model, "vision.resampler.output_norm", 1e-6f),
            dense(model, "vision.resampler.attention.qkv.weight"),
            dense(model, "vision.resampler.attention.qkv.bias"),
            dense(model, "vision.resampler.attention.output.weight"),
            dense(model, "vision.resampler.attention.output.bias"),
            dense(model, "vision.resampler.output.weight"));
    }
...
public:
    static WhisperAttention load(
        ...
        const bool fuse_qkv =
            fuse_value == nullptr || std::strcmp(fuse_value, "0") != 0;
        if (fuse_qkv) {
            auto query_bias = dense(model, prefix + ".query.bias");
            auto value_bias = dense(model, prefix + ".value.bias");
            auto weight = mlx::core::concatenate(
                {
                    dense(model, prefix + ".query.weight"),
                    dense(model, prefix + ".key.weight"),
        ...
        return WhisperAttention(
            MiniLinear::load(model, prefix + ".query"),
            MiniLinear::load(model, prefix + ".key", false),
            MiniLinear::load(model, prefix + ".value"),
            MiniLinear::load(model, prefix + ".output"));
    }
...
public:
    static WhisperLayer load(
        const MfqContainer& model,
        int index) {
        const auto prefix = "audio.block." + std::to_string(index);
        return WhisperLayer(
            WhisperAttention::load(model, prefix + ".attention"),
            MiniLayerNorm::load(
                model, prefix + ".attention.norm", 1e-5f),
            MiniLayerNorm::load(
                model, prefix + ".mlp.norm", 1e-5f),
            MiniLinear::load(model, prefix + ".mlp.up"),
            MiniLinear::load(model, prefix + ".mlp.down"));
    }
...
public:
    static AudioEncoder load(const MfqContainer& model) {
        ...
        return AudioEncoder(
            dense(model, "audio.patch_embedding.conv1.weight"),
            dense(model, "audio.patch_embedding.conv1.bias"),
            dense(model, "audio.patch_embedding.conv2.weight"),
            dense(model, "audio.patch_embedding.conv2.bias"),
            dense(model, "audio.position_embedding.weight"),
            std::move(layers),
            MiniLayerNorm::load(model, "audio.output_norm", 1e-5f),
            MiniLinear::load(model, "audio.projector.input"),
            MiniLinear::load(model, "audio.projector.output"));
    }
...
public:
    static TtsDecoder load(const MfqContainer& model) {
        ...
        return TtsDecoder(
            ...
            MlxEmbedding::load(model, "tts.text_embedding.weight"),
            MlxEmbedding::load(model, "tts.code_embedding.0.weight"),
            MiniLinear::load(model, "tts.semantic_projector.input"),
            MiniLinear::load(model, "tts.semantic_projector.output"),
            MiniLinear::load(model, "tts.speaker_projector.input"),
            MiniLinear::load(model, "tts.speaker_projector.output"),
            std::move(code_head));
    }
