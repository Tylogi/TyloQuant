...
// and packed NINT, NINT8-0, NVQ, NPQ, CCCP-I4G64, or CCCP product-VQ
// layouts. Expert-shaped/rotated NEPQ belongs to the MoE path and is rejected
// here. The decode-oriented kernel intentionally accepts only one through
// max_rows() flattened input rows. Callers should use their ordinary
...
public:
    ...
    // Production two/three-projection groups bind each source array directly
    // to one Metal dispatch. Larger NINT-only groups use the pooled
    // compatibility kernel; larger groups containing VQ or CCCP are
    // unsupported.
    bool uses_zero_copy_storage() const noexcept;
    std::size_t copied_packed_nbytes() const noexcept;

    // True when a float16, single-row invocation can use the NINT4/5/6
    // projection-fused kernel. The kernel assigns the same output tile to
    // every projection, so Q/K/V reuse each activation group in one dispatch.
