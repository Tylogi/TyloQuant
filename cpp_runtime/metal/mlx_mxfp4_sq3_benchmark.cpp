...
struct EncodedPair {
  std::vector<std::uint8_t> sq3_blob;
...
EncodedPair make_pair(int rows, int columns) {
  ...
  for (int row = 0; row < rows; ++row) {
    for (int state = 0; state < 8; ++state) {
      ...
      state_palettes[index] =
          static_cast<std::uint8_t>((row * 13 + state * 7) & 31);
    }
    for (int block = 0; block < blocks_per_row; ++block) {
      ...
      for (int lane = 0; lane < 31; ++lane) {
        ...
        const auto symbol = static_cast<std::uint8_t>(
            (row * 11 + block * 5 + lane * 3 + (lane >> 2)) & 7);
      ...
      symbols[last_index] = static_cast<std::uint8_t>(
          (((row + block) & 1) << 2) | (low_tag ^ required_low_tag));
      ...
      for (int lane = 0; lane < 32; ++lane) {
        const std::size_t value_index =
            static_cast<std::size_t>(row) * columns + block * 32 + lane;
        const auto nibble = mfq::metal::kMxfp4Sq3PaletteNibbles
            [static_cast<std::size_t>(palette) * 8 + symbols[value_index]];
  ...
  std::vector<std::uint8_t> sq3_blob{'S', 'Q', '3', '1'};
  append<std::uint8_t>(sq3_blob, 1);
  append<std::uint8_t>(sq3_blob, matrix_scale_base);
  append<std::uint16_t>(sq3_blob, 0);
  append<std::uint64_t>(sq3_blob, rows);
  append<std::uint64_t>(sq3_blob, columns);
  append_bytes(sq3_blob, pack_bits(symbols, 3));
  append_bytes(sq3_blob, pack_bits(selectors, 1));
  append_bytes(sq3_blob, pack_bits(state_scales, 2));
  append_bytes(sq3_blob, pack_bits(state_palettes, 5));
  ...
  return {std::move(sq3_blob), std::move(mxfp4_blob)};
}
  ...
  for (int row = 0; row < rows; ++row) {
    for (int column = 0; column < width; ++column) {
      ...
      values[index] = static_cast<float>(
          (row * 29 + column * 17 + 11) % 127 - 63) / 256.0f;
    }
  }
...
void verify_exact_weights(const MlxMxfp4Sq3Weight &sq3,
  ...
  mlx::core::eval(sq3_dense, native_dense);
  require(sq3_dense.size() == native_dense.size(), "dense size mismatch");
  for (std::size_t index = 0; index < sq3_dense.size(); ++index) {
    require(sq3_dense.data<float>()[index] == native_dense.data<float>()[index],
            "SQ3/native MXFP4 decoded-weight mismatch");
  }
}
...
void verify_matmul(const MlxMxfp4Sq3Weight &sq3, const MlxMxWeight &native,
  ...
  mlx::core::eval(sq3_output, native_output);
  float maximum_difference = 0.0f;
  for (std::size_t index = 0; index < sq3_output.size(); ++index) {
    maximum_difference = std::max(
        maximum_difference, std::fabs(sq3_output.data<float>()[index] -
                                      native_output.data<float>()[index]));
  }
  require(maximum_difference <= 0.125f,
          "SQ3/native MXFP4 matmul difference exceeds tolerance");
...
void print_trial(const char *format, const char *operation_name, int trial,
                 int rows,
                 std::size_t payload_nbytes, std::size_t output_nbytes,
                 int warmup, int repetitions, Operation &&operation) {
  const auto result =
  ...
  std::cout << format << '\t' << operation_name << '\t' << rows << '\t'
            << trial << '\t' << payload_nbytes << '\t' << repetitions << '\t'
            << std::fixed
            << std::setprecision(6) << result.milliseconds << '\t'
...
int main(int argc, char **argv) {
  try {
    ...
    verify_exact_weights(sq3, native);
    ...
    if (requested_rows == -1) {
      verify_matmul(sq3, native, input, 1);
      for (int trial = 0; trial < 3; ++trial) {
        ...
        const auto run_dequant = [&](bool run_native) {
          if (run_native) {
            ...
            print_trial("MXFP4-SQ3", "dequant_fp16", trial, 1,
                        sq3.packed_nbytes(), dense_nbytes, 2,
                        dequant_repetitions,
                        [&] { return sq3.dequantize(); });
          }
        };
        ...
        const auto run_gemv = [&](bool run_native) {
          if (run_native) {
            print_trial("MXFP4", "fused_gemv_fp16", trial, 1,
                        ...
                        gemv_repetitions,
                        [&] { return native.matmul(input); });
          } else {
            print_trial("MXFP4-SQ3", "fused_gemv_fp16", trial, 1,
                        sq3.packed_nbytes(), gemv_output_nbytes, 3,
                        gemv_repetitions,
                        [&] { return sq3.matmul(input); });
          }
        };
    ...
    for (const int input_rows : row_counts) {
      ...
      verify_matmul(sq3, native, multirow_input, input_rows);
      const auto output_nbytes = static_cast<std::size_t>(input_rows) * rows *
          sizeof(std::uint16_t);
      for (int trial = 0; trial < 3; ++trial) {
        for (int slot = 0; slot < 3; ++slot) {
          const int operation = (slot + trial) % 3;
          if (operation == 0) {
            print_trial("MXFP4-SQ3", "bucket_matmul_fp16", trial, input_rows,
                        sq3.packed_nbytes(), output_nbytes, 2,
                        multirow_repetitions,
                        [&] { return sq3.matmul(multirow_input); });
          } else if (operation == 1) {
            ...
            print_trial(
                "MXFP4-SQ3-DENSE", "dequant_matmul_fp16", trial, input_rows,
                sq3.packed_nbytes(), output_nbytes, 2, multirow_repetitions,
                [&] {
                  return mlx::core::matmul(
                      multirow_input,
                      mlx::core::transpose(sq3.dequantize()));
                });
          }
        }
      }
    ...
    std::cerr << "MFQ MXFP4-SQ3 Metal benchmark failed: " << error.what()
              << '\n';
    return 1;
  }
}
