...
common_peg_parser analyze_tools::build_parser(parser_build_context & ctx) const {
    switch (format.mode) {
        ...
        default:
            LOG_ERR("[ERROR] Template seems to support tool calls, but MFQ could not determine the tool format. "
                "Tool calling will not work properly; use a fixed template or report the unsupported template to MFQ.\n");
            return ctx.p.eps();
    }
}
