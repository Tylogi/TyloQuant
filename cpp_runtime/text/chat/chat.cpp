...
void common_chat_msg_delimiters::tokenize(const mfq_text_vocab * vocab) {
    for (auto & d : delimiters) {
        d.tokens = common_tokenize(vocab, d.delimiter, false, true);
    }
}

common_chat_msg_spans common_chat_msg_delimiters::split(const mfq_text_tokens & tokens, const std::map<size_t, size_t> & skips) const {
...
bool common_chat_verify_template(const std::string & tmpl, bool use_jinja) {
    ...
    mfq_text_chat_message chat[] = {
        { "user", "test" }
    };
    const int res = mfq_text_chat_apply_template(tmpl.c_str(), chat, 1, true, nullptr, 0);
    return res >= 0;
}
...
common_chat_templates_ptr common_chat_templates_init(const struct mfq_text_context * context,
    ...
    if (chat_template_override.empty()) {
        ...
        const auto * str = mfq_text_get_chat_template(context, /* name */ nullptr);
        if (str) {
            default_template_src  = str;
            has_explicit_template = true;
        }
        str = mfq_text_get_chat_template(context, /* name */ "tool_use");
        if (str) {
    ...
    bool        add_eos   = false;
    if (context) {
        const auto * vocab     = mfq_text_get_vocab(context);
        const auto   get_token = [&](mfq_text_token token, const char * name, const char * jinja_variable_name) {
        ...
        token_bos = get_token(mfq_text_vocab_bos(vocab), "BOS", "bos_token");
        token_eos = get_token(mfq_text_vocab_eos(vocab), "EOS", "eos_token");
        add_bos   = mfq_text_vocab_get_add_bos(vocab);
        add_eos   = mfq_text_vocab_get_add_eos(vocab);
    }
...
// Legacy template route (adhoc C++ implementation of known templates), forward to mfq_text_chat_apply_template.
static common_chat_params common_chat_templates_apply_legacy(const struct common_chat_templates *        tmpls,
                                                             const struct common_chat_templates_inputs & inputs) {
    size_t                          alloc_size = 0;
    std::vector<mfq_text_chat_message> chat;
    ...
    int32_t      res = mfq_text_chat_apply_template(src.c_str(), chat.data(), chat.size(), inputs.add_generation_prompt,
                                                 buf.data(), buf.size());

    // error: chat template is not supported
    if (res < 0) {
        // if the custom "tmpl" is not supported, we throw an error
        // this is a bit redundant (for good), since we're not sure if user validated the custom template with mfq_text_chat_verify_template()
    ...
    if ((size_t) res > buf.size()) {
        buf.resize(res);
        res = mfq_text_chat_apply_template(src.c_str(), chat.data(), chat.size(), inputs.add_generation_prompt, buf.data(),
                                        buf.size());
    }
