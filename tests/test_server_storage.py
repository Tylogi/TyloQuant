...
def test_media_is_content_addressed_deduplicated_and_persistent(tmp_path) -> None:
    store = make_store(tmp_path)
    data = b"small image payload"
    digest = hashlib.sha256(data).hexdigest()
    created = store.put_media(data, "image/png", digest, now=NOW)
    duplicate = store.put_media(data, "image/png", digest)
    assert duplicate.media == created.media
...
def test_session_fields_can_change_without_erasing_unpatched_values(tmp_path) -> None:
    store = make_store(tmp_path)
    ...
    renamed = store.update_session(
        SESSION_ID,
        UpdateSessionRequest(
            metadata={"assistant_id": "assistant:writer", "assistant_name": "Writer"}
        ),
        now=NOW,
    )
    assert renamed.mode == SessionMode.FULL_DUPLEX
    assert renamed.title == "conversation"
    assert renamed.metadata == {"assistant_id": "assistant:writer", "assistant_name": "Writer"}
